...
- Dedicated accessibility audit for keyboard-only traversal, roving keyboard navigation, broader screen-reader labels, and reduced motion. First-pass composer popover focus restoration and trigger expanded/collapsed ARIA state are now smoke-gated, but this does not close the whole-app accessibility pass.
