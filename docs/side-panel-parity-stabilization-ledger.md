...
- Main chat thread, composer, inspector/context panel, and Settings still need an explicit day-to-day coding usability audit and implementation pass. Codex parity evidence is useful here, but Orchestrator-owned workflow quality is the success bar. Local Personalization Settings now have a first pass with persisted custom instructions/coding preferences and focused smoke coverage; provider Settings now expose existing runtime capability gaps as compact boundaries; empty-chat starter actions, failed-run retry recovery, tool-failure retry recovery, first-pass assistant continuation, partial-response stopped recovery, queued follow-up cancellation, compact active-run queue summaries, and first-pass long-thread history/load/find ergonomics now have main-chat/composer coverage. Remaining main-chat/composer work should target richer active-run queue actions, model/context/permission workflow polish, exact live Codex long-thread timing only when live evidence is available, and real provider-backed retry/continue/partial-continue proof; remaining Settings work should target provider-native account/runtime adapters, real remote adapters, and higher-value daily coding workflows.
