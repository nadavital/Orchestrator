...
| Settings detail density | Shared settings rows either stretched controls across the whole right pane or, in Providers, opened into a cramped diagnostics dashboard. | Keep the Codex-style left nav/right detail structure, constrain the right detail column, keep simple controls close to their labels, and reserve broader diagnostics for the explicit Provider Details state. | `npm run smoke:ui:auto -- --settings`; `npm run smoke:ui:auto -- --settings-providers`; installed visual `tmp/side-panel-visual-inventory/settings-providers.png` |
