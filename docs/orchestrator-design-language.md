...
| Settings detail density | Providers and shared settings rows leaned on too many separators, making the right pane read like a diagnostics dashboard. | Reduce row chrome, soften settings separators, compact Provider diagnostics, and keep details secondary to Defaults. | `npm run smoke:ui:auto -- --settings-providers` |
