...
| Workbench tabs | Close buttons reserved dead space in tab labels, Browser and bottom Terminal tabs could collapse below their own names, and legacy Git tab state could reappear from saved UI state. | Retire `git` from right-panel state on normalization, route programmatic Git opens to Environment, and let durable right/bottom tabs use content-sized floors so short labels stay readable without slabby whitespace. | `npm run smoke:ui:auto -- --browser`; `npm run smoke:ui:auto -- --workbench-new-tab`; `npm run smoke:ui:auto -- --terminal-visual`; `npm run smoke:ui:auto -- --right-panel`; installed visual `tmp/side-panel-visual-inventory/browser.png` and `tmp/side-panel-visual-inventory/terminal-bottom-panel.png` |
| Settings detail density | Shared settings rows either stretched controls across the whole right pane or, in Providers, opened into a cramped diagnostics dashboard. | Keep the Codex-style left nav/right detail structure, constrain the right detail column, keep simple controls close to their labels, and move broader Provider diagnostics into an explicit Details dialog instead of extending the settings page. | `npm run smoke:ui:auto -- --settings`; `npm run smoke:ui:auto -- --settings-providers`; installed visual `tmp/side-panel-visual-inventory/settings-providers.png` |
| Left sidebar material | Sidebar had a strong right-edge tonal band, making the left rail read as a separate opaque strip instead of Codex-style translucent shell chrome. | Keep the sidebar translucent, remove the wide edge gradient, and use only subtle active-row state plus a low-opacity hairline where needed. | `node scripts/run-side-panel-visual-inventory.mjs --full --installed --only chat-sidebar,header`; installed visuals `tmp/side-panel-visual-inventory/chat-sidebar.png` and `tmp/side-panel-visual-inventory/header.png` |
| Composer controls | The composer toolbar had the right controls but the visual order and label pressure made it feel busy. | Keep branch/local as context, then order the execution cluster as attachment, permission mode, model/provider, send; keep provider menus translucent and compact, with accessible grouping instead of visible row labels inside the picker. | `npm run smoke:ui:auto -- --composer`; installed visual `tmp/side-panel-visual-inventory/composer.png` |
