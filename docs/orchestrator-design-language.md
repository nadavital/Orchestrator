...
| Settings detail density | Providers and shared settings rows leaned on too many separators and an artificially narrow right pane, making the page read like a cramped diagnostics dashboard. | Reduce row chrome, soften settings separators, widen the Provider detail canvas, compact Provider diagnostics, and keep details secondary to Defaults. | `npm run smoke:ui:auto -- --settings-providers`; installed visual `tmp/side-panel-visual-inventory/settings-providers.png` |
