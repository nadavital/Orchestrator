...
| Workbench tabs | Close buttons reserved dead space in tab labels, Browser and bottom Terminal tabs could collapse below their own names, and legacy Git tab state could reappear from saved UI state. | Retire `git` from right-panel state on normalization, route programmatic Git opens to Environment, and let durable right/bottom tabs keep readable name floors while close affordances avoid permanent dead space. | `npm run smoke:ui:auto -- --browser`; `npm run smoke:ui:auto -- --workbench-new-tab`; `npm run smoke:ui:auto -- --terminal-visual`; installed visual `tmp/side-panel-visual-inventory/browser.png` and `tmp/side-panel-visual-inventory/terminal-bottom-panel.png` |
