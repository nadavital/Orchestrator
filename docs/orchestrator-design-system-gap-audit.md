...
- Settings now shares design-system primitives for intro text, groups, panels, compact rows, choice cards, status pills, diagnostic pills, provider command-output cards, the provider picker, the provider model-list manager, and the provider config editor.
...
1. Finish the remaining settings local controls: color swatches and import controls.
