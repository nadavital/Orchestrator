...
- Settings now shares design-system primitives for intro text, groups, panels, compact rows, choice cards, status pills, diagnostic pills, provider command-output cards, the provider picker, and the provider model-list manager.
...
1. Finish the remaining settings local controls: color swatches, config editor, and import controls.
