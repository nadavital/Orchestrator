...
- Focused right-panel smoke now verifies `rightPanelFindStatusA11y=true` and `rightPanelFindScopeLabelA11y=true`, covering the shared chat/diff find bar's announced result-count status, input/status association, and scope-specific input labels.
