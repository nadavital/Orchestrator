...
- Current Orchestrator full visual inventory: `tmp/side-panel-visual-inventory-current/manifest.json`, 23 captures, no failures, created at `2026-05-26T07:00:05.999Z`.
...
1. Header/panel interaction parity: capture and fix how the right and bottom panels meet the titlebar/header, including top offsets, border continuity, toolbar height, and whether panel chrome starts below or visually integrates with the app header.
2. Browser agent-driven parity: implement or explicitly gate a dynamic client-tool bridge for browser-use style requests. Manual Browser UI is strong; provider/browser-use parity is still blocked at the current app-server boundary.
...
| --- | --- | --- | --- |
| Header and panel interaction | Live screenshot `/private/tmp/codex-current-screen.png` shows the Codex right-panel/review shell visually coupled to the app header/titlebar area, and the Codex bundle owns right/bottom panel focus, chrome, resize, and tab state through shared app-shell primitives. | Orchestrator now uses shared `AppShellPanel` primitives for right and bottom panels, but the header-to-panel seam has not been explicitly measured side by side. Top offset, border continuity, toolbar height, and whether panel chrome starts below or integrates with the app header remain open parity checks. | Make this the next visible UI parity slice: capture Codex and Orchestrator header/panel geometry, add smoke-readable geometry metadata where useful, then fix right-panel and bottom-panel seams from that evidence. |
| Chat sidebar top actions, width, and section order | Live screenshot `/private/tmp/codex-current-screen.png` shows the Codex sidebar starts with `New chat`, `Search`, `Plugins`, and `Automations` above top-level Pinned/Projects/Chats, and measures roughly 240 px wide. | Orchestrator now renders those four top utility rows above the sidebar sections, uses the shared sidebar row primitive, fixes the left sidebar width at 240 px instead of the old 300 px wide desktop layout, and renders Chats as a sibling section after Projects by default. Smoke gates order, placement, behavior, tightened width, projectless metadata, and the optional Chats-before-Projects preference. | Keep future sidebar visual work anchored in live screenshots; next sidebar gap is provider-backed pin/list mutation semantics or exact row spacing if a new live screenshot shows drift. |
...
3. For Browser, separate manual Browser panel parity from agent-driven browser-use parity: the Browser panel should stay provider-neutral, while agent-driven browser-use needs a real dynamic tool bridge or an explicit unavailable state per provider/runtime.
