...
| --- | --- | --- | --- |
| Chat sidebar top actions and width | Live screenshot `/private/tmp/codex-current-screen.png` shows the Codex sidebar starts with `New chat`, `Search`, `Plugins`, and `Automations` above Pinned/Projects/Chats, and measures roughly 240 px wide. | Orchestrator now renders those four top utility rows above the sidebar sections, uses the shared sidebar row primitive, and fixes the left sidebar width at 240 px instead of the old 300 px wide desktop layout. Smoke gates order, placement, behavior, and the tightened width. | Keep future sidebar visual work anchored in live screenshots; next sidebar gap is provider-backed pin/list mutation semantics or exact row spacing if a new live screenshot shows drift. |
