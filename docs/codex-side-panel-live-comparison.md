...
- Synthetic browser-use and review metadata events prove reducer/UI behavior, not that live Codex/provider sessions emit the same data in the app. The new `live:codex-browser-appserver` proof makes this explicit for Browser: the live app-server path works, but no browser-use surface is exposed to this client in the tested protocol.
