...
1. Browser agent-driven parity: build real Browser renderer dynamic tools on top of the now-proven app-server `dynamicTools` / `item/tool/call` transport before claiming provider parity. Manual Browser UI is strong, generic dynamic-tool transport works, and server-initiated unimplemented tool calls still surface an explicit unavailable status when no bridge exists.
