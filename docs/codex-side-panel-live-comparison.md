...
| --- | --- | --- | --- |
| Chat sidebar top actions | Live screenshot `/private/tmp/codex-current-screen.png` shows the Codex sidebar starts with `New chat`, `Search`, `Plugins`, and `Automations` above Pinned/Projects/Chats. | Orchestrator now renders those four top utility rows above the sidebar sections. They use the shared sidebar row primitive and are smoke-gated for order, placement, and behavior: New chat is wired to session creation, Search opens the command palette, Plugins opens Capabilities, and Automations opens Settings > Automations. | Keep future sidebar visual work anchored in live screenshots; next sidebar gap is provider-backed pin/list mutation semantics or exact row spacing if a new live screenshot shows drift. |
