...
| Review mode | Partial | `enteredReviewMode` and `exitedReviewMode` normalize into typed `review.mode.changed` state, persist through transcript status rows, and surface in the Plan tab with an Open Review action. Live `npm run live:codex-review-start` proves native app-server `review/start` accepts `target: { type: "uncommittedChanges" }` with inline delivery and emits review-mode items. Codex-backed Review panels now expose a `Start Codex review` action for that proven uncommitted inline path. |
...
| Review | `review/start` | Supported for the proven uncommitted inline path through `sessions.startCodexReview` and the Codex app-server runtime. Base branch, commit, custom, and detached paths remain unwired. |
...
| Review | Partial/external | Existing provider command surface can use headless review command; review-mode items are parsed, native `review/start` uncommitted inline flow is live-proven, and Codex Review panels can start that native uncommitted inline flow. | Product controls for base/commit/custom targets, detached review handling, and richer review-mode item UI. |
...
3. Codex Review Mode:
   - Status: `enteredReviewMode` and `exitedReviewMode` now parse into typed review-mode state, persist through transcript status rows, and surface in the right Workbench Plan tab with an Open Review action. `npm run live:codex-review-start` proves the native uncommitted inline `review/start` contract and timing against the real app-server, and Codex Review panels can start that native path.
   - Remaining: add base/commit/custom target controls, wire detached review handling, and keep the live proof opt-in for Review slices instead of routine smoke.
