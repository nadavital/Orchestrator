...
- Browser boundary proof:
  ...
  - `npm run live:codex-dynamic-tools`
  - `tmp/codex-dynamic-tools-live-proof/result.json`
  - `npm run live:codex-browser-tools`
  - `tmp/codex-browser-tools-live-proof/result.json`
  - Focused Browser UI smoke with `browserClientToolBridge=true`
...
| Review mode | Partial | `enteredReviewMode` and `exitedReviewMode` normalize into typed `review.mode.changed` state, persist through transcript status rows, and surface in the Plan tab with an Open Review action. Live `npm run live:codex-review-start` proves native app-server `review/start` accepts uncommitted, base-branch, and commit targets with inline delivery and emits review-mode items. Codex-backed Review panels now expose native start actions for the proven uncommitted, branch, and commit Review paths. |
...
| Review | `review/start` | Supported for the proven uncommitted inline, base-branch inline, and commit inline paths through `sessions.startCodexReview` and the Codex app-server runtime. Custom and detached paths remain unwired. |
...
| `item/tool/call` | Supported/partial | Supported Browser client tools route through the dynamic Browser bridge and answer with `DynamicToolCallResponse` content items. Unsupported client tools still emit a visible `Client tool unavailable` status and respond with a structured JSON-RPC unsupported-tool error. The live dynamicTools proof (`tmp/codex-dynamic-tools-live-proof/result.json`) confirms app-server can send `item/tool/call` with JSON-RPC id `0` for an advertised client tool and complete the turn when answered; the live Browser tools proof (`tmp/codex-browser-tools-live-proof/result.json`) confirms Codex requests real `orchestrator.browser_open` and `orchestrator.browser_read` tools in sequence; renderer-backed Browser open/read is unit-covered and focused-smoke-covered. Native provider-emitted browser-use event streaming is still pending. |
...
| Review | Partial/external | Existing provider command surface can use headless review command; review-mode items are parsed, native `review/start` uncommitted, base-branch, and commit inline flows are live-proven, and Codex Review panels can start those native paths. | Product controls for custom targets, detached review handling, and richer review-mode item UI. |
...
3. Codex Review Mode:
   - Status: `enteredReviewMode` and `exitedReviewMode` now parse into typed review-mode state, persist through transcript status rows, and surface in the right Workbench Plan tab with an Open Review action. `npm run live:codex-review-start` proves the native uncommitted, base-branch, and commit inline `review/start` contracts against the real app-server, and Codex Review panels can start those native paths.
   - Remaining: add custom target controls, wire detached review handling, and keep the live proof opt-in for Review slices instead of routine smoke.
