...
| Browser-use client tools | Partial/blocked | Codex desktop has browser-use webview state and turn-route capture/release code in the app bundle, but the live browser-use proof completes without `browser.manager_state`, browser server requests, or dynamic browser tool calls. A separate live dynamicTools proof confirms the stdio app-server accepts advertised `dynamicTools`, sends `item/tool/call`, and completes when Orchestrator responds. Manual Orchestrator Browser UI remains provider-neutral; agent-driven browser-use automation still needs real Browser renderer tools on that transport. |
...
| `item/tool/call` | Parsed/bridge-proven | Orchestrator emits a visible `Client tool unavailable` status and responds with a structured JSON-RPC unsupported-tool error in the production runtime. The live dynamicTools proof (`tmp/codex-dynamic-tools-live-proof/result.json`) confirms app-server can send `item/tool/call` with JSON-RPC id `0` for an advertised client tool and complete the turn when answered. The live browser-use proof still sees no browser-specific tool request, so browser-use is missing a renderer tool bridge, not just generic transport support. |
...
| Browser-use client tools | Partial/blocked | Manual Browser panel state, tabs, navigation, inspection, comments, security policy, and local-target discovery are Orchestrator-owned and provider-neutral. The Codex desktop bundle has browser-use client route evidence, and the live dynamicTools proof confirms the app-server transport can advertise and answer dynamic client tools. The live browser-use proof still emits no browser events or browser-specific dynamic tool requests. If a server requests an unimplemented client dynamic tool, the unsupported boundary remains visible in the transcript and test-covered. | Real Browser renderer dynamic tools advertised through `dynamicTools` and routed from `item/tool/call` to the Browser renderer. Keep explicit unavailable state for runtimes/providers without that bridge. |
