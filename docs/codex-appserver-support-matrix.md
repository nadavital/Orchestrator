...
| `item/tool/call` | Parsed/blocked | Orchestrator emits a visible `Client tool unavailable` status and responds with a structured JSON-RPC unsupported-tool error instead of leaving the server request hanging or hiding it in runtime diagnostics. The live browser proof currently sees no `item/tool/call` requests, so browser-use is not merely missing a handler in this path. |
...
| Browser-use client tools | Blocked | Manual Browser panel state, tabs, navigation, inspection, comments, security policy, and local-target discovery are Orchestrator-owned and provider-neutral. The Codex desktop bundle has browser-use client route evidence, but the live stdio app-server proof emitted no browser events or dynamic browser tool requests. If a server does request a client dynamic tool, the unsupported boundary is now visible in the transcript and test-covered. | A real dynamic client-tool bridge that advertises browser tools and routes tool calls to the Browser renderer. Keep explicit unavailable state for runtimes/providers without that bridge. |
