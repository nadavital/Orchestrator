...
Codex evidence: the installed Codex bundle keeps bottom-panel size in shared app-shell state initialized at `400`, while the Terminal bottom-panel chunk renders the terminal surface below shared bottom-panel chrome. Orchestrator already had focused Terminal behavior smoke for this, but the runnable visual comparison only required solid material and healthy terminal content, which left the Codex-sized bottom-panel target out of the directly comparable screenshot path.

Implemented: the `terminal-visual` smoke path now computes the same content/chrome/total size decomposition as the focused Terminal behavior smoke and exposes `terminalBottomPanelSizeDecomposition=true` when the visual bottom panel is 350 px content plus 50 px chrome for a 400 px shared shell target. The side-panel comparison now requires that check in the `Terminal bottom panel` row, and the header/panel interaction ledger calls out the bottom-panel size as part of the coordinated shell contract.
