...
| Header and panel interaction | The runnable comparison now treats sidebar top inset, main session header ownership, compact icon-only profile/debug affordance, Review/Workbench right-panel header seam, shared sidebar/main/right-panel header-band height, title-first tooltip-backed metadata behavior, right/bottom panel material, Terminal bottom-panel attachment, and Terminal's Codex-bundle-aligned 280 px bottom-shell size as one shell contract. | User-observed risk: Orchestrator can still feel unlike Codex when panels and the header are validated independently. The active mismatch to watch is the relationship between the left sidebar top band, the main titlebar/header row, Review/Workbench right-panel chrome, and bottom-panel attachment as one coordinated shell. Exact live Codex pixel spacing and panel animation timing remain unverified, so a green individual panel smoke is not enough to close this workstream. | `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` includes a dedicated `Header and panel interaction` row backed by live screenshot evidence plus `sidebarTopInsetCodexLike=true`, `profileBadgeCompact=true`, `headerActionChromeCompact=true`, `sessionHeaderInPrimaryColumn=true`, `rightPanelHeaderSeam=true`, `headerPanelSharedBand=true`, `headerMetadataTooltipOnly=true`, `rightPanelMaterialSolid=true`, `terminalPanelMaterialSolid=true`, `terminalBottomPanelSizeDecomposition=true`, and `terminalVisualHealthyContent=true` across `chat-sidebar`, `header`, `transcript-narrow`, `workbench-right-panel`, `review-last-turn`, `review-core`, and `terminal-bottom-panel` captures. The same command writes `tmp/codex-side-panel-comparison/header-panel-contact-sheet.html`, grouping the live Codex screenshot with Orchestrator's sidebar, focused header, transcript/header, Workbench right-panel, Review panel, and Terminal bottom-panel captures. Latest manifest: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-26T17:19:21.442Z`; latest comparison report: `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-26T17:19:21.720Z`, with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`. The current live screenshot file is flagged `stale/incomplete` because its PNG nonblank metrics are `nonBlack=0`, `lumaStdDev=0`, and `colors=1`. | Keep this as a first-class parity workstream. Before claiming any sidebar, Review, Workbench, main titlebar, or bottom Terminal change is complete, verify the header/panel row, inspect the contact sheet, and add a concrete mismatch if Codex shows a different header/panel relationship. |
