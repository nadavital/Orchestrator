...
Implemented: added a host-scoped Orchestrator `Browser` Settings section using the shared Settings page/content/surface/row primitives. The page clears real in-app Browser data for the selected local host partition through the existing Electron `browser:clearData` path, with individual rows for all browsing data, cookies, site data, and cached images/files. Approval, history, and domain policy rows are intentionally explicit as session-scoped Browser panel Security controls rather than fake global settings, because Orchestrator does not yet have durable global Browser Use policy persistence.
...
Remaining: this closes the missing Settings Browser page/data-control gap. Global Browser Use approval/history/domain persistence, remote-host Browser settings adapters, and live Codex Settings screenshot comparison remain open.
