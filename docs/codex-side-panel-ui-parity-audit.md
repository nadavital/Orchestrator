...
Working rule for the current goal: panel/header interaction is a parity surface of its own. A sidebar, Workbench, Review, Files, Browser, or Terminal change is not done just because its focused smoke passes; it also needs to preserve the shared Codex-like relationship between the left sidebar top band, the main session header, the Files/Browser/Review/Workbench right-panel chrome, and the bottom panel. Use the dedicated comparison row and `tmp/codex-side-panel-comparison/header-panel-contact-sheet.html` before treating cross-panel shell work as complete; if the contact sheet shows the header and panels behaving like separate stacked surfaces, promote that as an active mismatch before doing more isolated panel work.

| Surface | Current parity state | Important remaining mismatches | Verification state | Recommended next move |
| --- | --- | --- | --- | --- |
| App shell / panel ownership | `AppShellPanel`, shared panel markers, focus-area attributes, shared animated size/progress/mounted metadata for right/bottom panels, right/bottom tabpanel ownership, resize handles, tab lifecycle telemetry, product-level panel open/close telemetry, shared right/bottom tab context-menu sections, a shared Workbench side-panel layout controller, a shared Terminal bottom-panel layout controller, a shared transfer capability resolver, session-level `transferSessionPanelTab`, right/bottom terminal move/receive semantics, explicit unsupported-transfer metadata for non-terminal right-to-bottom moves, Browser-focused address/reload/back/forward commands, focused Browser/right-Terminal/bottom-Terminal `Cmd+T` routing, Codex-style shared thread find for chat/diff scopes with Browser-focused `open-find` routing, a native menu enabled-state bridge for focused panel commands, app-owned active-window command targeting, per-window native-menu availability isolation, window-scoped pending navigation for new session windows, loaded-window deep-link pending cleanup, and shared open Browser / toggle Browser / open Review commands exist. The full side-panel inventory now includes the multi-window focus/menu-routing smoke as a first-class shell capture instead of a detached note. | Route/window lifecycle and exact live Codex shell spacing/timing remain incomplete; non-terminal bottom-panel rendering is now an explicit product/runtime decision because the current Codex bottom-panel bundle is terminal-specific; raw macOS `BrowserWindow.getFocusedWindow()` still does not switch back in the automated smoke even though Orchestrator's app-owned active-window handoff restores command state and routes menu commands correctly. | `npm run build` passes. `git diff --check` passes. `npm run test:providers` passes. Focused right-panel smoke passes with `rightPanelTransferUnsupportedBoundary=true` across Review/Files/Browser, `rightPanelSharedAnimationController=true`, `rightPanelSharedLayoutController=true`, `rightPanelContextMenuSharedSections=true`, `rightPanelPanelOpenCloseTelemetry=true`, `rightPanelFindShortcutRouting=true`, `rightPanelBrowserCommandRouting=true`, `rightPanelMenuCommandState=true`, plus shell ownership, tab focus, telemetry, close-active, resize reset, fullscreen cleanup, expand, and overlay checks. `rightPanelFindShortcutRouting=true` now proves the shared `content-search-input` with Search chat/Search diffs scope plus Browser special routing, not the older per-panel Review search focus. Latest right-panel panel-lifecycle evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779850119191.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779850119191.png`. The refreshed full comparison also gates `secondWindowCreated=true`, `secondWindowNavigated=true`, `pendingNavigationConsumedOnce=true`, `pendingNavigationWindowScoped=true`, `loadedDeepLinkDoesNotLeavePendingNavigation=true`, `firstWindowBrowserFocusArea=true`, `firstWindowBrowserMenuEnabled=true`, `secondWindowBrowserMenuDisabled=true`, `backgroundWindowMenuDoesNotClobberFocusedWindow=true`, `activeWindowAfterRefocus=true`, `focusSwitchRestoresFirstWindowMenu=true`, and `menuCommandRoutedToFocusedWindow=true`; latest multi-window evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-multi-window-focus-1779852428108.json`; screenshot: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/multi-window-focus.png`. Raw OS-level focus proof remains separate. Full dev side-panel comparison now passes with 27 captures and no failures; manifest `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-27T03:28:15.204Z`; comparison report `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T03:28:15.455Z`. | High priority remains broader route/window lifecycle, exact live Codex width/height/timing comparison, and any product-backed non-terminal bottom-panel renderer decision. Keep installed-app proof green after release-level shell changes. |
| Header and panel interaction | The runnable comparison now treats sidebar top inset, main session header ownership, compact icon-only profile/debug affordance, Files/Browser/Review/Workbench right-panel header seam, shared sidebar/main/right-panel header-band height, title-first tooltip-backed metadata behavior, right/bottom panel material, Terminal bottom-panel attachment, Terminal's Codex-bundle-aligned 280 px bottom-shell size, app-owned multi-window menu command routing, and loaded-window deep-link handoff as one shell contract. | User-observed risk: Orchestrator can still feel unlike Codex when panels and the header are validated independently. The active mismatch to watch is the relationship between the left sidebar top band, the main titlebar/header row, Files/Browser/Review/Workbench right-panel chrome, bottom-panel attachment, and window/menu focus ownership as one coordinated shell. Exact live Codex pixel spacing, OS focus behavior, and panel animation timing remain unverified, so a green individual panel smoke is not enough to close this workstream. | `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` includes a dedicated `Header and panel interaction` row backed by live screenshot evidence plus `sidebarTopInsetCodexLike=true`, `profileBadgeCompact=true`, `headerActionChromeCompact=true`, `sessionHeaderInPrimaryColumn=true`, `rightPanelHeaderSeam=true`, `headerPanelSharedBand=true`, `filesHeaderPanelSeam=true`, `browserHeaderPanelSeam=true`, `headerMetadataTooltipOnly=true`, `rightPanelMaterialSolid=true`, `terminalPanelMaterialSolid=true`, `terminalBottomPanelSizeDecomposition=true`, `terminalVisualHealthyContent=true`, `secondWindowCreated=true`, `secondWindowNavigated=true`, `pendingNavigationConsumedOnce=true`, `pendingNavigationWindowScoped=true`, `loadedDeepLinkDoesNotLeavePendingNavigation=true`, `firstWindowBrowserFocusArea=true`, `firstWindowBrowserMenuEnabled=true`, `secondWindowBrowserMenuDisabled=true`, `backgroundWindowMenuDoesNotClobberFocusedWindow=true`, `activeWindowAfterRefocus=true`, `focusSwitchRestoresFirstWindowMenu=true`, and `menuCommandRoutedToFocusedWindow=true` across `chat-sidebar`, `header`, `transcript-narrow`, `workbench-right-panel`, `files`, `browser`, `review-last-turn`, `review-core`, `terminal-bottom-panel`, and `multi-window-focus` captures. The same command writes `tmp/codex-side-panel-comparison/header-panel-contact-sheet.html`, grouping the live Codex screenshot with Orchestrator's sidebar, focused header, transcript/header, Workbench right-panel, Files panel, Browser panel, Review panel, Terminal bottom-panel, and multi-window shell captures. Latest dev manifest: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-27T03:28:15.204Z`; latest comparison report: `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T03:28:15.455Z`, smoke app mode `dev`, with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, `needsProof=0`, and 27 smoke captures. The comparison can attempt a fresh macOS live capture with `--capture-live-codex`; the latest escalated attempt exited `0` but still flagged `/private/tmp/codex-current-screen.png` as `stale/incomplete` because its PNG nonblank metrics are `nonBlack=0`, `lumaStdDev=0`, and `colors=1`. | Keep this as a first-class parity workstream. Before claiming any sidebar, Files, Browser, Review, Workbench, main titlebar, bottom Terminal, or window-routing change is complete, verify the header/panel row, inspect the contact sheet, and add a concrete mismatch if Codex shows a different header/panel relationship. |
| Smoke infrastructure signal | The runnable comparison now distinguishes product/UI mismatches from smoke-start failures. | A sandbox/local permission failure such as `listen EPERM 127.0.0.1` can prevent every focused smoke from producing screenshots/check payloads. That is not evidence that all side panels regressed; it is missing smoke evidence. | `run-side-panel-visual-inventory.mjs` records `failureKind` and `failureSummary` per capture. `run-codex-side-panel-comparison.mjs` reports those rows as `needs-smoke` when the failure kind is infrastructure, while completed assertion failures still count as `mismatch`. | When the comparison reports `needs-smoke` with infrastructure failure kinds, rerun through the approved localhost-capable command path before changing UI. Only chase `mismatch` rows that have real smoke checks or visual evidence. |
| Workbench tab/controller chrome | Shared `PanelTabStrip` covers toolbar metrics, measured trailing action reserve, Codex-style start-edge hover/focus close, middle-click close, active scroll, tabpanel focus, shimmer, basic reorder, before/after drag insertion markers, shared tab context-menu sections, provider-neutral `workbench-panel-*` wrapper/test selectors, shared right-panel animation/progress metadata, a Codex-style `New tab` surface with action cards for Files, Side chat, Browser, Review, and Terminal, keyboard/ARIA resizing through the shared left-edge separator, a right-panel visual reset that returns Browser to normal single-tab chrome after command-routing checks, and an explicit non-terminal right-to-bottom unsupported-transfer boundary. | Non-terminal bottom-panel renderers remain unsupported by current Codex bottom-panel evidence; exact live layout timing is still missing; active background and shell material are close but not fully Codex-owned. `ContextSidebar` remains an internal component name until a larger route/window ownership cleanup. | Focused right-panel smoke passes with `workbenchPanelChromeCompact=true`, `workbenchPanelTabCloseStartEdge=true`, `workbenchPanelNewTabPage=true`, `rightPanelSharedAnimationController=true`, `rightPanelContextMenuSharedSections=true`, `rightPanelTransferUnsupportedBoundary=true`, `rightPanelTabDragMarker=true`, `rightPanelResizeKeyboard=true`, `rightPanelBrowserVisualReset=true`, plus resize, overlay, close, drag, focus routing, and Browser command routing checks. Latest evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779822056924.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779822056924.png`. Dedicated New Tab evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-workbench-new-tab-1779621364872.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-workbench-new-tab-1779621364872.png`. Latest full visual inventory passed with `workbench-right-panel.png` and `workbench-new-tab.png`: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-26T19:06:50.755Z`. | High priority after shell ownership. Consolidate remaining Workbench chrome toward exact active/overflow/chrome visual comparison and route/window lifecycle cleanup; do not invent non-terminal bottom-panel renderers without product-backed Codex evidence. |
| Review / Changes entry | Inline `Edited N files` transcript card now uses a Codex-like file-first patch shape with per-file status/path/stats rows, expandable inline diff preview, a truthful local-current-change `Undo`, source-aware provider last-turn patch rendering from normalized `diff.updated` events, provider session/turn/checkpoint metadata preservation, and a direct `Review` action that opens detailed Review while preserving a sibling Environment Workbench tab. Environment is a first-class Workbench tab with Codex-like `Environment`, `Changes`, `Local`, branch, `Commit`, `Create pull request`, and `Sources / Web search` rows; `Changes` opens detailed Review, `Commit` routes into Review when local changes exist, and PR metadata turns the PR row into a real `View pull request` external-open action. Review can render provider-agnostic PR/check/reviewer metadata affordances in the compact toolbar, the metadata flyout now uses shared menu section/row primitives, and sessions can hydrate that metadata from a real GitHub PR through the optional `gh pr view` adapter. | True provider checkpoint Undo is still missing; provider last-turn transcript cards now distinguish missing checkpoint ids from unsupported checkpoint adapters and remain disabled instead of falling back to local discard. Non-GitHub/provider-native hosted adapters still do not populate PR/check/reviewer metadata; Environment create-PR generation remains unavailable without provider/gh metadata; exact spacing and right-rail content need live side-by-side comparison. | `npm run build` passes. `git diff --check` passes. `npm run test:providers` passes with Git discard, diff parser, GitHub PR metadata normalization, and Codex app-server provider checkpoint metadata coverage. Latest `npm run smoke:ui:auto -- --diff-last-turn` passes with `reviewTranscriptCardLastTurn=true` and `reviewLastTurnVisualState=true`, proving a Codex-comparable Last turn Review screenshot with source-level stats and the changed-files rail hidden; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-last-turn-1779800711978.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-last-turn-1779800711978.png`. Latest `npm run smoke:ui:auto -- --diff-source` passes and covers provider Last turn source rendering, Last turn Copy git apply, provider session/turn metadata on the transcript Review card, Worktree provider source, source search, comments/blame, full-source selected-line and gutter blame, and hidden context gates. Evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-source-1779771068089.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-source-1779771068089.png`. Latest `--diff-entry` evidence covers local-current `reviewTranscriptCardUndo`, direct Review entry, shared metadata flyout, metadata toolbar, and Environment panel with `reviewMetadataFlyoutShared=true`: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-entry-1779653681761.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-entry-1779653681761.png`. Dedicated Environment visual smoke also passes with `environmentPanelVisual=true`, `environmentActionRows=true`, and `environmentSources=true`; inventory screenshot: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/environment.png`. | Keep Review backlog, but stop expanding it as the universal proving ground. Next Review work should target real provider checkpoint restore only after a provider supplies checkpoint id plus rollback API, non-GitHub/provider-native PR metadata adapters, or one verified visual mismatch at a time. |
| Review diff workspace | Split/unified diffs, selected-file main rendering, file headers, line gutters, hunk collapse, hidden-context expansion, word diff, hide whitespace, local comments, selected-line blame, source-tab handoff, right-side changed-files rail, diff-content search filtering with match-count badges, `Cmd+F` focus to the changed-files search when Review is focused, All/Worktree/Staged/Branch/Commit sources, provider-agnostic Last turn source from normalized `diff.updated` events with provider session/turn/checkpoint metadata, transcript-card Last turn patch rendering, session-backed Local provider source rows, worktree-backed provider source rows when the session is in a worktree, full-source row/gutter mode through the shared `FilePreview` source renderer with selected-line, bounded gutter blame, and source-virtualization metadata, rich preview, shared `PanelNotice` binary fallback/action states, centered no-change empty state, deterministic loading state, narrow overlay Review containment, an in-panel Review header with visible source/stat summary, Review-level `Copy git apply command` over the currently loaded local or Last turn patch set, a Codex-style panel-root floating local Revert all / Stage all / Unstage all action pill with confirmation, Codex-threshold large-diff bounded rendering, and merge-conflict file metadata/badges plus basic current/incoming/both marker-block action helpers are implemented. | Cloud provider-native review source remains unavailable until there is a real cloud diff adapter; provider-backed hosted review-source flows are missing; true provider checkpoint/last-turn restore is still missing because no adapter supplies checkpoint id plus rollback API; full Codex-style DOM virtualization/buffers remain incomplete; PR/reviewer-backed comments/blame are missing; deeper merge-conflict mark-resolved/stage workflows and hosted conflict adapters are not implemented; live Codex side-by-side spacing proof remains open. | `npm run build` passes. `git diff --check` passes. `npm run test:providers` passes, including `review diff parser derives file changes and per-file diff bodies` plus provider checkpoint metadata normalization. `pnpm test src/main/__tests__/gitChanges.test.ts` passes with real merge-conflict `UU` normalization and conflict-marker diff coverage. Focused `npm run smoke:ui:auto -- --diff-conflict` passes with `reviewMergeConflictHelpers=true`, proving a Codex-style `merge-conflict-action-*` slot plus Current/Incoming/Both actions and marker-block rewrite; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-conflict-1779675864451.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-conflict-1779675864451.png`. `--diff-core`, `--diff-source`, `--diff-preview`, `--diff-empty`, `--diff-loading`, and `--diff-narrow` now run against isolated Review states and pass. Latest core evidence after the conflict-helper slice: JSON `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-core-1779675933405.json`; screenshot `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-core-1779675933405.png`. Latest focused source evidence covers Local provider source availability for a local session, disabled cloud/worktree rows when unsupported, Last turn becoming selectable after a synthetic normalized `diff.updated` event, rendering the Last turn patch in both detailed Review and transcript Review card, provider session/turn metadata plus missing-checkpoint Undo state on the transcript card, `reviewLastTurnGitApplyCommand=true` for Copy git apply over that loaded provider diff, `reviewSearchContent=true` for a query that appears only in diff content, `reviewFullSourceRows=true` and `reviewFullSourceBlame=true` through the shared source renderer, `reviewWorktreeProviderSource=true` for an app-managed worktree session with Worktree enabled and Local disabled, and `reviewLargeDiffWindow=true` for limiting a 15,202 changed-line diff at Codex-scale thresholds while ordinary 3k-row diffs no longer trigger the large-diff notice: JSON `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-source-1779771068089.json`; screenshot `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-source-1779771068089.png`. Focused Review preview evidence covers `reviewFallbackNoticeShared=true`, binary action states, and rich preview modes: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-preview-1779645748850.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-preview-1779645748850.png`. Focused right-panel smoke also proves Review-focused `Cmd+F` routes to `diff-file-search`. Latest loading evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-loading-1779674876904.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-loading-1779674876904.png`. Latest narrow evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-narrow-1779632866664.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-narrow-1779632866664.png`. Empty-state evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-empty-1779674872016.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-empty-1779674872016.png`. | Medium priority. Review mechanics are now split-gated; next Review work should continue one visible or provider-backed mismatch at a time: real provider checkpoint restore after checkpoint id plus rollback API exists, cloud provider diff adapters, hosted provider source flows, real PR metadata adapters beyond the local GitHub path, provider-backed comments/blame, full DOM diff virtualization, hosted/deeper conflict workflows, and live Codex side-by-side spacing comparison. |
| Files / file viewer | Files is now a Workbench browser plus first-class preview/pinned file tabs. Host-aware ids, sticky folders, lazy directories, filtered ancestor projection, selected-row reveal, structured previews, notebook read-only controls, notebook output rendering, notebook `details`/`summary` cell disclosure shells, notebook execution-count/position header labels, notebook metadata title/description rendering, notebook output summary markdown, notebook raw-output disclosures, text/plain-before-JSON notebook MIME precedence, notebook Python-titled code snippets, Codex-like notebook cell spacing/width, Codex-like notebook output/source chrome, Codex-like stream/text output item chrome, separate markdown/error/image notebook item chrome, PDF artifact header chrome, Codex-style artifact Open/options header controls, Codex-style Workbench file-source action labels and compact options/open action cluster, shared `content-search-input` source find in diff scope, workbook-style XLSX sheet tabs, read-only XLSX active-cell/formula-bar chrome, rich/source mode, Codex-shaped artifact import-kind/type metadata and source-option boundaries, `Cmd+F` focus routing to Files/workspace search or shared source find when focused, bounded content-hit search with line metadata, local annotations, selected-line reveal, selected-line actions, bounded gutter blame, shared action/context menu sections, shared Workbench file-tab action-menu sections, shared `PanelNotice` binary/unreadable fallback action states, and a shared Codex-like Workbench tree message primitive for Files empty/loading/no-match rows exist. | External PR/reviewer-backed comments/blame metadata is missing; exact shared-find focus timing and global/provider-indexed workspace search are still basic; standalone/no-session split preview remains as fallback; full Codex artifact renderer fidelity for spreadsheet/document/slides/PDF remains incomplete, editable spreadsheet behavior is not implemented, and provider-backed artifact metadata remains open. | Focused Files smoke passes with `filesPdfPreview=true`, `filesArtifactTabModel=true`, `filesPreviewHeaderShared=true`, `filesArtifactPreviewControls=true`, `filesArtifactOpenOptions=true`, `workbenchFileTabCodexActionLabels=true`, `workbenchFileTabCodexActionCluster=true`, `fileSourceSearch=true`, `filesSpreadsheetSheetTabs=true`, `filesSpreadsheetActiveCell=true`, `filesContentSearch=true`, `filesNotebookReadOnlyControls=true`, `filesNotebookOutputRendering=true`, `filesNotebookCellDisclosure=true`, `filesNotebookExecutionCount=true`, `filesNotebookCellMetadata=true`, `filesNotebookOutputSummaries=true`, `filesNotebookRawOutputDisclosure=true`, `filesNotebookCodeSnippet=true`, `filesNotebookCellSpacing=true`, `filesNotebookOutputChrome=true`, `filesNotebookOutputItemChrome=true`, `filesNotebookRichOutputItemChrome=true`, `filesActionMenuSharedSections=true`, `filesRowContextMenuSharedSections=true`, `workbenchFileTabActionMenuSharedSections=true`, `filesFallbackNoticeShared=true`, `filesNoResults=true`, and existing tree/search/source/preview gates. Latest focused spreadsheet active-cell evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779844791812.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779844791812.png`. Latest full-inventory Files evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779844922268.json`; screenshot: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/files.png`. Focused right-panel smoke also proves `Cmd+F` routes to workspace file search and Browser find from focused Workbench tabs. | Medium priority. Future work should focus on full artifact rendering, provider-backed comments/blame, provider/global indexed search, or exact shared-find focus/timing polish rather than local tree polish. |
...
| Terminal / bottom panel | Terminal has dedicated `TerminalPanel`, shared `PanelTabStrip`, tabpanel focus, telemetry, shared tab context-menu sections, drag/context reorder with the same before/after insertion marker as Workbench, close-active cleanup, link routing into Browser, shell ownership markers, shared bottom-panel layout metadata/clamping, shared animated size/progress metadata, shared edge-marked Codex-style overlay resize separators backed by a shared zoom-normalized resize controller with keyboard/ARIA support, smoke-proven top-edge drag/reset without layout displacement, terminal moves routed through the shared session transfer action, right/bottom move/receive actions with generic `Move tab to ... panel` menu copy, focused right/bottom `Cmd+T` new-tab routing, live theme/font option sync from Appearance tokens into xterm, a Codex-matched VS Code terminal token matrix, service snapshots, shared app-shell header chrome, Codex-matched xterm content spacing/token surface, Codex-bundle-aligned 280 px default total bottom-panel size, and a pipe-backed shell fallback when `node-pty` cannot start. | Exact live Codex open/close animation timing remains incomplete. Native PTY startup still depends on the local environment; the fallback keeps the panel functional instead of rendering a failure overlay. | `npm run smoke:ui:auto -- --terminal` passes with `terminalSharedAnimationController=true`, `terminalSharedLayoutController=true`, `terminalBottomPanelSizeDecomposition=true`, `terminalTabMenuSharedSections=true`, `terminalTabDragMarker=true`, `terminalHeaderSharedChrome=true`, `terminalContentSpacing=true`, `terminalResizeReset=true`, `terminalResizeHandleOverlay=true`, `terminalResizeKeyboard=true`, tab persistence, shell ownership, restore, menu/reorder/drag, shortcuts, a11y, fullscreen cleanup, telemetry, lifecycle telemetry, `terminalSharedTransferModel=true`, `terminalMoveToRightPanel=true`, `terminalMoveBackToBottom=true`, right-panel new-tab shortcut, link routing, `terminalThemeFontSync=true`, `terminalThemeTokenMatrix=true`, and `terminalServiceSnapshot=true`. Latest focused evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-1779822058884.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-1779822058884.png`. The current implementation targets 230 px terminal content plus 50 px chrome for a 280 px total shell, and `--terminal-visual` gates the same `terminalBottomPanelSizeDecomposition=true` with `terminalVisualHealthyContent=true`. The full comparison now includes a separate `terminal-behavior` capture so transfer checks are directly comparable while the stable `terminal-bottom-panel` screenshot remains visual-only. | Medium priority. Next terminal slice should be live Codex animation timing comparison or broader root shell transfer/rendering decisions for non-terminal tab kinds. |
| Chat Sidebar | Local/remote/cloud/remote-host/worktree identity, Codex-style provider-pinned thread-key normalization/application, local pinned-row drag reorder with persisted session pin orders, Codex app-server `thread/list` metadata projection for array/`data`/`threads`/`items` plus nested `conversation`/`task`/`pendingWorktree` shapes, non-blocking startup sync wiring, debounced post-run app-server sync wiring, manual Provider Settings refresh wiring, throttled idle/focus recurring refresh wiring, provider-pinned merge metadata, provider projectless thread-id metadata, provider worktree source/root/host metadata, pending-worktree lifecycle, projectless Chats grouping, user-facing projectless Chats-first preference, provider connection grouping, label-color metadata, automation row metadata, custom/built-in section order, collapsed sections, local custom section membership, drag/drop into custom sections, in-section drag reorder, section-level custom-section reorder with insertion markers, text-first session rows, footer collapse/expand affordance with a 48 px collapsed rail, shared session/project/organize/custom-section action-menu sections, Codex-compact row density, and central selected-sidebar key persistence exist. | Provider adapters still need live provider pin list/set RPCs where available, richer projectless/worktree/hosted source reconciliation from real provider sources beyond the current thread-list metadata adapter, and the richer Codex action set. Current Codex app-server live probing shows pinned bridge calls are not exposed through app-server and still return unknown-variant errors, so provider pin RPC parity is externally blocked unless a different bridge/state boundary is used; the new drag reorder is a local Orchestrator pin-order boundary. Exact live Codex spacing remains open unless a new screenshot identifies a measurable drift beyond the compact/text-first/footer-collapse density contract. | Sidebar pieces are covered by broad smoke and unit tests. Latest focused Sidebar smoke passes with `sidebarFooterCollapseAffordance=true`, `sidebarRowDensityCodexLike=true`, `sessionRowsTextFirst=true`, `sidebarPinnedRowsTextFirst=true`, `sidebarPinnedDragReorder=true`, `sidebarSelectedKeySignal=true`, `sidebarSelectedKeyPersistence=true`, `sidebarSelectedNavKeys=true`, `providerWorktreeMetadata=true`, `providerProjectlessMetadata=true`, `providerPinnedMetadata=true`, `sidebarActionMenuSharedSections=true`, `projectActionMenuSharedSections=true`, `organizeMenuSharedSections=true`, `sidebarConnectionGrouping=true`, and `sidebarPinActionsConsolidated=true` over local, remote, cloud, remote-host, worktree, provider-hosted worktree, and pending-worktree fixtures; latest evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779803856358.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779803856358.png`. Focused unit coverage also passes for Codex `items` and nested local conversation, remote task, pending worktree, and hosted-conversation metadata projection through the built `node --test` path. Settings-provider smoke passes with `settingsProviderSidebarRefresh=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-providers-1779640324525.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-providers-1779640324525.png`. Latest full visual inventory passed with 25 captures and refreshed `chat-sidebar.png`; manifest: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-26T14:03:25.425Z`. | High priority for visible product feel. Next Sidebar work should choose a safe alternative state boundary for provider pin set/list, deeper real provider worktree/projectless adapters beyond thread-list metadata, or live Codex side-by-side visual comparison if a safe screenshot path is available. |
| Settings | Shared settings page/surface/group/row primitives exist across General, Appearance, Providers, Automations, Worktrees, Shortcuts, Pet overlay, Host Personalization, Browser, and Data controls. All local Settings pages now use a shared Codex-style `SettingsContentLayout` content header for title/subtitle above the scroll body. The Browser Settings page is backed by Codex `browser-use-settings-*` bundle evidence and exposes real host-scoped in-app Browser data clearing for all data, cookies, site data, and cache, while keeping approval/history/domain policy explicitly session-scoped in the Browser panel Security inspector. Shortcut editing/conflict behavior, custom multi-binding, per-binding clear/remove, disabled-default binding state, shifted punctuation/code normalization, shared shortcut row icon-button actions, a compact Codex-like read-only shortcut capture input, and reset behavior exist. Settings now owns shared route parsing/building for Codex-style `/settings/<section>?host=...` paths in HTTP/dev renderers and packaged `orchestrator-app://renderer/` main windows, keeps the old `#/settings/<section>` parser as a direct `file://` fallback, opens sections from the route, syncs Settings nav changes back to that route, exposes route-owned settings-shell metadata, and keeps Back-to-chat behavior route-aware. The left settings nav mirrors Codex's visible App / Host grouping while keeping shared sidebar row primitives: Pet overlay is app-scoped, while Personalization and Browser are host-scoped. A Host-section nav action renders the provider-host selector when remote-host metadata exists, persists the selected host id, normalizes stale selections back to Local, filters remote-host navigation away from local-only pages, redirects local-only active sections back to General for remote hosts, and remounts Settings content under the selected host key. Settings content now uses shared typed app/host scope and host-adapter-state helpers, so app-scoped pages on a remote host are marked `app-global` while host-scoped pages without a provider adapter render an explicit unavailable state instead of local controls. Personalization is explicitly smoke-gated as host-scoped and unavailable until Orchestrator has a Codex-style memory/personality/custom-instructions adapter; Pet overlay keeps the app-specific pet controls without masquerading as Codex Personalization. Provider Settings now exposes a compact Codex-only manual `Refresh chats` action that refreshes Codex sidebar thread metadata through the app-server without starting a run. Worktrees, Automations, and Data controls destructive confirmations now use the shared `ConfirmDialog`/dialog-layout path instead of native `window.confirm`. | Remote-host-specific page behavior now has explicit unavailable boundaries for host-scoped pages without adapters, but is not backed by real remote settings adapters; Codex Personalization specifically is host-scoped memory, personality, and custom instructions; Browser Use approval/history/domain policy is still per-session until Orchestrator has durable global policy persistence; deeper native theme/font/chrome controls, platform-specific shortcut capture live edge cases, and larger sheet-style editors still need live comparison. | Settings focused smoke passes with `settingsContentLayout=true`, `settingsRouteOwned=true`, `settingsSidebarGroupedNav=true`, `settingsSidebarNavPrimitive=true`, `settingsTopbarShared=true`, `settingsHostContext=true`, `settingsHostSectionFiltering=true`, `settingsHostAdapterBoundary=true`, `settingsPersonalizationHostBoundary=true`, `settingsBrowserPage=true`, `settingsBrowserSurface=true`, `settingsBrowserModule=true`, `settingsShortcutsPerBindingClear=true`, `settingsShortcutsPunctuationCapture=true`, `settingsShortcutActionsShared=true`, `settingsShortcutCaptureFieldShared=true`, and all existing Settings page gates. Latest Settings evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-1779854070171.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-1779854070171.png`. Latest packaged Settings deep-link smoke passes with `settingsDeepLinkRoute=true` and `orchestrator-app:` path-route debug: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-deeplink-1779681835187.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-deeplink-1779681835187.png`. Dedicated Pet overlay smoke passes with `petsSettingsContentLayout=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-pets-1779678586608.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-pets-1779678586608.png`. Settings-provider smoke passes with `settingsRouteOwned=true` and `settingsProviderSidebarRefresh=true`; latest evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-providers-1779669653727.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-providers-1779669653727.png`. | Medium/high priority. Next Settings slice should add a real host-scoped Personalization adapter, add real remote-host settings adapters for other visible host sections, deepen Browser Use approval/history/domain persistence if product-backed, deepen native theme/font/chrome controls, do platform-specific shortcut capture/live Codex comparison, or inspect larger sheet-style editors. |
...
| Verification infrastructure | Broad and focused smokes cover many surfaces. `reviewFullSourceRows`, `reviewEnvironmentPanel`, `environmentPanelVisual`, `environmentActionRows`, `reviewEmptyState`, `reviewLoadingState`, `reviewNarrowOverlay`, Files source checks, Browser shell-tab checks, Browser command routing, native menu command-state checks, Terminal behavior checks, `terminalThemeFontSync`, `settingsHostContext`, `settingsHostSectionFiltering`, `settingsShortcutsPerBindingClear`, `--terminal-visual`, `terminal-behavior`, `--workbench-new-tab`, `--environment`, `--multi-window-focus`, Sidebar text-first row/footer-collapse/selected-key checks, Settings grouped-nav checks, shell tab checks, right-panel shared-layout checks, right-panel panel open/close telemetry checks, Review split smoke aliases, packaged right-panel smoke, packaged side-panel inventory mode, installed side-panel inventory mode, and a full local side-panel screenshot/behavior inventory exist. The Settings visual endpoint now leaves an actual Settings page visible for capture, Settings smoke now mutates and clears a specific shortcut binding, Settings smoke now seeds remote-host metadata, verifies that the host selector is owned by the Host navigation section, verifies remote-host filtering for local-only Settings pages, and then switches back to Local to keep Automations/Worktrees/Data coverage. Terminal visual inventory uses a real visual-only checkpoint with healthy terminal content instead of the destructive behavior smoke or failure overlay, Terminal behavior smoke now mutates appearance tokens and proves xterm theme/font sync plus transfer semantics, the New Tab visual endpoint captures the action-card page directly, the Environment visual endpoint captures the git/PR action rows directly, `--multi-window-focus` now gates window-scoped pending navigation, loaded-window deep-link pending cleanup, app-owned active-window restoration, and command routing back to the first window, broad smoke restores the previously active Workbench tab after proving New Tab, and the comparison tool has a `--capture-live-codex` option that attempts `/private/tmp/codex-current-screen.png` capture before validating PNG nonblank metrics. The visual-inventory wrapper now launches each focused capture directly with a process-group timeout and still treats completed output JSON plus a copied screenshot with all checks passing as valid, so a child process that writes evidence but hangs during shutdown no longer prevents a comparable manifest. | Live Codex side-by-side/pixel review is still needed before claiming final visual parity. Installed `/Applications/Orchestrator.app` replacement verification is current for the loaded-window deep-link shell lifecycle slice through a full elevated `--installed` comparison with 27 captures and no failures. Raw macOS `BrowserWindow.getFocusedWindow()` still does not report the first window after automated refocus, so true OS-level manual/live focus proof remains separate even though app-shell command targeting is now verified. Computer Use could not inspect `com.openai.codex`, and latest `--capture-live-codex` attempts still produce a black screenshot in this environment, so exact Codex pixel comparison remains unavailable. The legacy monolithic `--diff` path may still carry stale sequencing assumptions and should not be used as the only Review verdict. Native PTY startup can still be environment-dependent, but the visual gate now proves a functional fallback content state instead of accepting the terminal error panel. | `npm run build` currently passes. `npm run test:providers` passes. `git diff --check` passes. Latest focused Sidebar smoke passes with `sidebarFooterCollapseAffordance=true`, `sessionRowsTextFirst=true`, `sidebarPinnedRowsTextFirst=true`, `sidebarRowDensityCodexLike=true`, `providerPinnedMetadata=true`, `sidebarPinActionsConsolidated=true`, and `hoverCardMaterial=true`; packaged evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779848313353.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779848313353.png`. Latest focused right-panel smoke passes with `rightPanelPanelOpenCloseTelemetry=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779850119191.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779850119191.png`. Latest full side-panel visual inventory passed for 27 captures with no failures; manifest: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-27T03:28:15.204Z`. Latest packaged core side-panel visual inventory passed for 8 captures with no failures; manifest: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-packaged-core/manifest.json`, created at `2026-05-27T02:21:23.400Z`. Latest installed comparison report passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`, created at `2026-05-27T03:41:51.077Z`; installed manifest created at `2026-05-27T03:37:08.348Z`. Latest multi-window smoke passed with `pendingNavigationWindowScoped=true`, `loadedDeepLinkDoesNotLeavePendingNavigation=true`, `activeWindowAfterRefocus=true`, `focusSwitchRestoresFirstWindowMenu=true`, and `menuCommandRoutedToFocusedWindow=true`; installed evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-multi-window-focus-1779852980419.json`; dev evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-multi-window-focus-1779852428108.json`. | Compare live/bundle Codex screenshots against the local screenshot matrix before claiming final visual parity. Keep Terminal behavior and visual evidence as separate checks. Keep Review work targeted to provider-native sources, real PR metadata adapters, or one verified visual mismatch at a time. Use `--packaged` for temp-bundle parity proof and `--installed` only when `/Applications/Orchestrator.app` exists and installed replacement proof is required. |
...
- Orchestrator now shows the active Review source plus source-level additions/deletions in the in-panel header and keeps full source selection in Review options; remaining source gaps are real cloud/hosted provider adapters and exact live Codex spacing around branch/commit rows.
...
- Workbench and Terminal now share Codex-like tab-to-panel ownership, focus routing, wheel scrolling for overflowed tab rows, hover/focus and middle-click close affordances, fullscreen-owner cleanup, close-active fallback from the main focus area, tab opened/closed/viewed telemetry, and shared panel opened/closed telemetry. Later slices also add right/bottom terminal move semantics and focused find routing. Larger shell-controller gaps remain: route/window lifecycle, exact live timing, and panel shortcuts beyond close/find.
...
- `AppShellPanel` centralizes panel markers, shell material, shared right/bottom open/close telemetry, and later slices cover the shared right/bottom size model, terminal move/receive, and focused find routing. The root shell still needs broader route/window lifecycle proof, exact live timing, and remaining panel keyboard arbitration beyond close/find.
...
| Codex logs side panel open/close/viewed telemetry. | Orchestrator now records shared `panel.opened`, `panel.closed`, `panel.tab.opened`, `panel.tab.closed`, and `panel.tab.viewed` metrics for Workbench and Terminal tabs, with panel id, surface, active tab, route kind, tab id, and tab count metadata. | Shared tab and panel lifecycle telemetry is covered; broader route/window lifecycle analytics remain incomplete. |
...
### 2026-05-26 - Provider Diff Checkpoint Metadata Boundary Slice

Codex evidence: the Codex app-server protocol exposes `turn/diff/updated` as the provider-owned Last turn diff event and has a separate not-yet-wired `thread/rollback` request family. The previous Orchestrator boundary collapsed every provider Last turn Undo case into generic unsupported state, even when the provider event carried enough turn identity to explain what was missing.

Implemented: normalized `diff.updated` events now preserve provider session id, provider turn id, optional checkpoint id, and explicit `checkpointUndoSupported` metadata. The transcript Last turn Review card exposes those values through smoke-visible attributes and distinguishes `data-review-card-provider-checkpoint-undo="missing-checkpoint"` from adapter-level unsupported checkpoint restore. The button remains disabled for provider Last turn cards, and local git discard is still reserved for local current-change cards only.

Verification: `pnpm exec tsc --noEmit` passed. `npm run test:providers` passed with 292 tests, including Codex app-server provider turn/checkpoint metadata coverage. `git diff --check` passed. `npm run build` passed. Focused `npm run smoke:ui:auto -- --diff-source` first failed inside the sandbox with `listen EPERM 127.0.0.1`; the escalated run passed with `reviewTranscriptCardLastTurn=true`, `reviewLastTurnGitApplyCommand=true`, and all existing diff-source gates while asserting provider session id, provider turn id, empty checkpoint id, adapter-supported false, disabled Undo, and the missing-checkpoint title. Evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-source-1779771068089.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-source-1779771068089.png`. `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=7`, `aligned=2`, `mismatch=0`, and `blocked=0`.
...
Codex evidence: re-checked the current extracted Codex bundle at `/private/tmp/orchestrator-codex-app-asar-20260524`. `webview/assets/app-shell-panel-animation-C6SMnz6V.js` animates a shared target panel size, `webview/assets/thread-page-bottom-panel-state-D1Lz0U4Y.js` keeps the Terminal surface as a full-height app-theme child below shared bottom-panel chrome, and the root app-shell bottom-panel path in `webview/assets/app-shell-Bh-lgoQk.js` stores `app-shell:bottom-panel-height` with a 280 px fallback, clamps through `Tn`, and passes `defaultSize: 280` to the top-edge resize handle.

Implemented: Terminal now exposes content height, chrome height, total shell height, and default/reset height metadata on the bottom app-shell panel, the bottom-panel chrome, and the active tab panel. The focused Terminal smoke now gates `terminalBottomPanelSizeDecomposition=true` by checking that the app-shell animation target is the total panel height, that total height equals content plus chrome, that the content tab panel's rendered height matches the resolved Terminal content height, and that the reset/default target is a Codex-bundle-aligned 280 px total panel height.

Verification: `pnpm exec tsc --noEmit` passed. Focused `npm run smoke:ui:auto -- --terminal` first failed inside the sandbox with `listen EPERM 127.0.0.1`; the escalated run passed with `terminalBottomPanelSizeDecomposition=true` plus existing terminal animation, layout, resize, transfer, service snapshot, shortcut, link-routing, and theme gates. Evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-1779778325710.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-1779778325710.png`. Terminal visual smoke passed with `terminalVisualHealthyContent=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-visual-1779778349082.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-visual-1779778349082.png`.

Remaining: this closes the local Terminal content/chrome/default-size ambiguity and gives future live Codex comparisons exact dimensions to compare. It does not yet prove live Codex pixel-level animation timing curves, route/window lifecycle cleanup, bottom rendering for Browser/Files/Review tabs, installed-app replacement proof, or any remaining Review backlog.
...
Implemented: Orchestrator's left sidebar now renders the same top utility stack above its chat sections. The rows use the shared sidebar row primitive and wire into existing app behavior: New chat creates a session, Search opens the command palette, Plugins opens the Capabilities surface, and Automations opens Settings > Automations. The previous single `Capabilities` top row is now represented by the Codex-shaped `Plugins` entry while preserving the existing Capabilities surface. Follow-up visual corrections fixed the left sidebar width at 240 px instead of the previous 300 px desktop layout and changed the New chat / Plugins icons to Codex-like compose and grid icons.

Verification: `pnpm exec tsc --noEmit` passed. Focused `npm run smoke:ui:auto -- --sidebar` passed after moving the new behavioral probes to the end of the long sidebar smoke so they no longer perturb existing project/action checks. The smoke now gates `sidebarPrimaryActionsCodexOrder=true`, `sidebarPrimaryActionsBeforePinned=true`, `sidebarSearchPrimaryAction=true`, `sidebarPluginsPrimaryAction=true`, `sidebarAutomationsPrimaryAction=true`, and a tightened 240 px `sidebarWidthToken=true`. Evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779775263541.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779775263541.png`.
...
Implemented: reduced Orchestrator's left sidebar drag spacer from 64 px to 48 px, exposed it as `data-testid="sidebar-window-drag-spacer"`, and added a focused `sidebarTopInsetCodexLike` smoke assertion. The runnable comparison script now includes a dedicated `Header and panel interaction` contract across `chat-sidebar`, `transcript-narrow`, `workbench-right-panel`, and `terminal-bottom-panel`, requiring `sidebarTopInsetCodexLike=true`, `sessionHeaderInPrimaryColumn=true`, `rightPanelHeaderSeam=true`, and `terminalVisualHealthyContent=true`.
...
### 2026-05-26 - Header/Main Shell Contract Tightening

Codex evidence: user review called out that panel/header interaction still needs to stay on the parity list because Codex and Orchestrator can diverge when panels are treated as independent panes. The current live Codex screenshot and bundle evidence remain the same source of truth: the sidebar, main conversation header, right panel, and bottom panel are coordinated app-shell surfaces.

Implemented: tightened the comparison contract so the `Header and panel interaction` row also includes the `transcript-narrow` main-shell capture. The transcript-layout smoke now asserts `sessionHeaderInPrimaryColumn=true`, proving the session titlebar/header remains owned by the primary chat column and aligned with the main session row. The comparison row now gates sidebar inset, main header ownership, right-panel seam, and terminal bottom-panel content together.

Verification: `pnpm exec tsc --noEmit` passed. Focused `npm run smoke:ui:auto -- --transcript-layout` initially hit the expected sandbox `listen EPERM 127.0.0.1:5173`; the escalated rerun passed with `sessionHeaderInPrimaryColumn=true`, evidence JSON `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-transcript-layout-1779782002589.json`, and screenshot `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-transcript-layout-1779782002589.png`. Full `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` first exposed a transient `settings-providers` runtime-status race; focused `--settings-providers` then passed, and the second full comparison passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`. `git diff --check` and `npm run build` passed.
...
### 2026-05-26 - Notebook Artifact Cell Disclosure

Codex evidence: `tmp/codex-app-assets/notebook-preview-panel-CAO-aRhM.js` renders notebook cells as open `details` sections with `summary` headers inside the read-only notebook preview.

Implemented: Orchestrator's structured notebook preview now renders each cell as an open `details` element with a summary header, chevron disclosure marker, body wrapper, source block, and preserved output rendering. The focused Files smoke now verifies that notebook cells are actual `HTMLDetailsElement` nodes, start open, carry `data-notebook-cell-disclosure="true"`, and expose both summary/header and body structure.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, and `git diff --check` passed. Focused `npm run smoke:ui:auto -- --files` passed after the expected escalated rerun with `filesNotebookCellDisclosure=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779791009968.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779791009968.png`. The refreshed `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 23 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, and the Files comparison row now includes `filesNotebookCellDisclosure`.
...
### 2026-05-26 - Notebook Artifact Execution Count

Codex evidence: `tmp/codex-app-assets/notebook-preview-panel-CAO-aRhM.js` preserves code-cell `execution_count`, renders `Cell {cellNumber} of {totalCellCount}` in cell headers, shows `Run {executionCount}` for executed code cells, and exposes a disabled per-cell run affordance because notebook previews are read-only.

Implemented: Orchestrator's structured notebook preview now preserves notebook `execution_count`, exposes cell position metadata, renders `Cell N of M`, shows `Run N` for executed code cells, and adds a disabled per-cell run control. The automated notebook fixture now sets `execution_count: 7` on the updated code cell, and the focused Files smoke gates this through `filesNotebookExecutionCount=true`.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` passed with `filesNotebookExecutionCount=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779806945035.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779806945035.png`. Refreshed `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 25 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needs-smoke=0`, and `needs-proof=0`; manifest created at `2026-05-26T14:54:12.281Z`.

Remaining: this closes the execution-count/header-label mismatch in the notebook artifact preview. It does not claim exact Codex cell spacing/output sizing, live provider artifact metadata, or runnable notebook execution.

### 2026-05-26 - Notebook Artifact Cell Metadata

Codex evidence: `tmp/codex-app-assets/notebook-preview-panel-CAO-aRhM.js` searches direct cell metadata and nested Codex metadata keys for `title` / `cellTitle` / `cell_title` and `descriptionMarkdown` variants. It uses metadata titles in notebook cell headers and renders described code cells as description markdown plus a nested read-only Code disclosure for the source.

Implemented: Orchestrator's structured notebook preview now preserves direct and nested Codex notebook cell metadata, uses metadata titles in cell headers, renders code-cell description markdown, and moves source into a nested Code disclosure when a description exists. The automated notebook fixture now includes nested `metadata.codex.title` and `metadata.codex.descriptionMarkdown`, and the focused Files smoke gates this through `filesNotebookCellMetadata=true`.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` passed with `filesNotebookCellMetadata=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779807591115.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779807591115.png`. Refreshed `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 25 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needs-smoke=0`, and `needs-proof=0`; manifest created at `2026-05-26T15:05:07.379Z`.

Remaining: this closes the notebook metadata-title/description mismatch. It does not claim exact Codex cell spacing/output sizing, live provider artifact metadata, or runnable notebook execution.

### 2026-05-26 - Notebook Artifact Output Summaries

Codex evidence: `tmp/codex-app-assets/notebook-preview-panel-CAO-aRhM.js` reads `outputSummaries[index].summaryMarkdown` from direct and nested Codex cell metadata, attaches it to stream, error, text, and JSON notebook outputs, and renders the markdown summary before raw output details.

Implemented: Orchestrator's structured notebook preview now preserves output summary metadata, attaches summaries to supported output types, and renders summary markdown above raw stream/text/JSON/error output content. The automated notebook fixture now includes two nested `metadata.codex.outputSummaries` entries, and focused Files smoke gates this through `filesNotebookOutputSummaries=true`.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` passed with `filesNotebookOutputSummaries=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779808216031.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779808216031.png`. Refreshed `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 25 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needs-smoke=0`, and `needs-proof=0`; manifest created at `2026-05-26T15:15:24.289Z`.

Remaining: this closes the notebook output-summary metadata mismatch. It does not claim exact Codex output spacing/sizing, live provider artifact metadata, or runnable notebook execution.
...
### 2026-05-26 - Shared Header Band Contract

Codex evidence: live screenshot `/private/tmp/codex-current-screen.png` shows the Codex sidebar, conversation header, and right-panel chrome occupying one compact shell band rather than separate independently sized header strips. The existing bundle evidence in `app-shell-state-HP0T5lEX.js` and `thread-page-bottom-panel-state-D1Lz0U4Y.js` keeps this tied to shared right/bottom app-shell state instead of a per-panel visual tweak.

Implemented: compacted Orchestrator's session titlebar into a single-line shell band and introduced `--app-shell-header-height` as the shared token for the primary session header and sidebar drag spacer. The metadata remains available in the header tooltip/inline truncated text but no longer forces a tall two-line primary header. The focused right-panel smoke now gates `headerPanelSharedBand=true`, proving the sidebar spacer, primary titlebar, and Workbench chrome use the same compact header-band height while preserving the existing `rightPanelHeaderSeam=true` ownership check.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, and `git diff --check` passed. Focused `npm run smoke:ui:auto -- --header` passed with `headerIdentity=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-header-1779796593818.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-header-1779796593818.png`. Focused `--sidebar` passed with `sidebarTopInsetCodexLike=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779796642940.json`. Focused `--transcript-layout` passed with `sessionHeaderInPrimaryColumn=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-transcript-layout-1779796711267.json`. Focused `--right-panel` passed with `rightPanelHeaderSeam=true` and `headerPanelSharedBand=true`; final evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779797165555.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779797165555.png`. Refreshed `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 23 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needs-smoke=0`, and `needs-proof=0`; manifest created at `2026-05-26T12:11:00.811Z`; report: `tmp/codex-side-panel-comparison/comparison-report.json`; header/panel contact sheet: `tmp/codex-side-panel-comparison/header-panel-contact-sheet.html`.

Remaining: this closes the concrete mismatch where the main header, sidebar spacer, and Workbench chrome could pass separate checks while still feeling visually inconsistent. Exact live Codex pixel spacing and panel animation timing remain open until fresh live screenshot evidence gives a measurable target.

### 2026-05-26 - Title-First Header Metadata Boundary

Codex evidence: live screenshot `/private/tmp/codex-current-screen.png` shows Codex's session header as a compact title-first band rather than a visible multi-field provenance strip. After the shared header-band slice, Orchestrator still rendered project/folder/branch/provider metadata inline next to the title, making the compact band visually busy and unlike Codex even though the height/seam checks passed.

Implemented: Orchestrator now keeps the visible session header title-first and moves project/folder/branch/provider metadata to the title tooltip and a screen-reader-only `session-header-metadata` node. The chat actions menu remains the actionable provenance surface for copying folder/project/session/provider ids. Focused header and right-panel smokes now gate `headerMetadataTooltipOnly=true`, and the header/panel comparison row includes this check so future header work cannot reintroduce a visible provenance strip into the compact shell band.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, and `git diff --check` passed. Focused `npm run smoke:ui:auto -- --header` passed with `headerIdentity=true`, `headerMetadataTooltipOnly=true`, and `headerActionMenu=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-header-1779797813818.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-header-1779797813818.png`. Focused `--right-panel` passed with `rightPanelHeaderSeam=true`, `headerPanelSharedBand=true`, and `headerMetadataTooltipOnly=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779797813265.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779797813265.png`. Refreshed `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 23 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needs-smoke=0`, and `needs-proof=0`; manifest created at `2026-05-26T12:24:17.184Z`; report: `tmp/codex-side-panel-comparison/comparison-report.json`; header/panel contact sheet: `tmp/codex-side-panel-comparison/header-panel-contact-sheet.html`.

Remaining: this closes the concrete title/provenance crowding mismatch in the compact header band. Exact live Codex title spacing, profile/badge behavior in non-smoke profiles, and animation timing remain live-visual comparison items.

### 2026-05-26 - Codex-Compact Sidebar Row Density

Codex evidence: live screenshot `/private/tmp/codex-current-screen.png` shows Codex's left sidebar as a compact rail, and the installed Codex bundle CSS maps `text-sm` to 12 px. Orchestrator's previous sidebar rows used 13 px title text and a looser 28 px row target, which made the left rail feel heavier even after the 240 px width, top action order, section order, and header-band work landed.

Implemented: Orchestrator now tags the left sidebar with `data-sidebar-density="codex-compact"`, uses 12 px row text, 11 px secondary metadata, 26 px nav/thread rows, 22 px compact rows, and tighter section row padding. Automated smoke profiles also force graphite/dark appearance unless explicitly opted out with `ORCHESTRATOR_AUTOMATED_UI_SMOKE_PRESERVE_THEME=1`, so the direct visual inventory stays comparable with the current dark Codex screenshot instead of drifting with the host system appearance.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, and `node -c scripts/run-codex-side-panel-comparison.mjs` passed. Focused `npm run smoke:ui:auto -- --sidebar` passed with `sidebarRowDensityCodexLike=true`, `sessionRowsCompact=true`, `sessionRowsCalm=true`, and dark comparable screenshot evidence; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779798887562.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779798887562.png`. Refreshed `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 23 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needs-smoke=0`, and `needs-proof=0`; manifest created at `2026-05-26T12:40:10.737Z`; report: `tmp/codex-side-panel-comparison/comparison-report.json`; header/panel contact sheet: `tmp/codex-side-panel-comparison/header-panel-contact-sheet.html`.

Remaining: this closes the measurable row-density drift in the current left sidebar screenshot path. Exact live Codex row spacing at other window sizes and provider-backed pin/list mutation remain open.

### 2026-05-26 - Terminal Bottom-Panel Visual Size Gate

Codex evidence: the installed Codex bundle keeps bottom-panel sizing in the shared root app-shell path, where `app-shell-Bh-lgoQk.js` stores `app-shell:bottom-panel-height` with a 280 px fallback and passes `defaultSize: 280` to the resize handle, while the Terminal bottom-panel chunk renders the terminal surface below shared bottom-panel chrome. Orchestrator already had focused Terminal behavior smoke for this, but the runnable visual comparison only required solid material and healthy terminal content, which left the Codex-sized bottom-panel target out of the directly comparable screenshot path.

Implemented: the `terminal-visual` smoke path now computes the same content/chrome/total size decomposition as the focused Terminal behavior smoke and exposes `terminalBottomPanelSizeDecomposition=true` when the visual bottom panel is 230 px content plus 50 px chrome for a 280 px shared shell target. The side-panel comparison now requires that check in the `Terminal bottom panel` row, and the header/panel interaction ledger calls out the bottom-panel size as part of the coordinated shell contract.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, and `git diff --check` passed. Focused `npm run smoke:ui:auto -- --terminal-visual` first hit the expected sandbox `listen EPERM 127.0.0.1:5173`; the escalated rerun passed with `terminalBottomPanelSizeDecomposition=true`, `terminalPanelMaterialSolid=true`, and `terminalVisualHealthyContent=true`. Evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-visual-1779799780155.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-visual-1779799780155.png`. Refreshed `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` first reported `needs-smoke=10` from sandbox infrastructure failures; the escalated rerun passed with 23 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needs-smoke=0`, and `needs-proof=0`; manifest created at `2026-05-26T12:54:47.652Z`; report: `tmp/codex-side-panel-comparison/comparison-report.json`; header/panel contact sheet: `tmp/codex-side-panel-comparison/header-panel-contact-sheet.html`.

Remaining: this strengthens local comparison proof for bottom-panel size. Exact live Codex open/close animation timing and live height across multiple window sizes still require new live Codex screenshots or installed-app inspection.

### 2026-05-26 - Last Turn Review Visual Capture

Codex evidence: live screenshot `/private/tmp/codex-current-screen.png` shows a transcript Review card opening the right-panel Review state with `Last turn`, source-level diff stats, file header stats, compact diff rows, and no persistent changed-files rail. The previous Orchestrator behavior smoke proved this state during `--diff-source`, but the visual inventory ended in worktree/local Review states with the rail visible, so the comparison did not have a directly comparable screenshot.

Implemented: added a focused `--diff-last-turn` smoke view and `review-last-turn.png` visual inventory capture. The smoke injects a provider `diff.updated` Last turn patch, opens the transcript Review card, leaves Review in `last-turn` source with the changed-files rail hidden, and gates `reviewLastTurnVisualState=true`. Review source summary now shows source-level additions/deletions such as `Last turn +1` through `data-review-source-summary-additions/deletions`, matching the Codex source-row stats pattern while keeping file counts in the accessible label and source menu.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-side-panel-visual-inventory.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, and `git diff --check` passed. Focused `npm run smoke:ui:auto -- --diff-last-turn` first hit the expected sandbox `listen EPERM 127.0.0.1`; the escalated rerun passed with `reviewTranscriptCardLastTurn=true` and `reviewLastTurnVisualState=true`. Evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-last-turn-1779800711978.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-last-turn-1779800711978.png`. Refreshed `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 24 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needs-smoke=0`, and `needs-proof=0`; manifest created at `2026-05-26T13:25:56.260Z`.
...
Implemented: removed the visual leading label-color marker from `SessionItem` while preserving provider/thread/label-color metadata on the row shell. Pin/unpin remains in the consolidated trailing action slot, and running/unread/error/automation indicators still render as right-slot status controls. Focused Sidebar smoke now gates `sessionRowsTextFirst=true` and `sidebarPinnedRowsTextFirst=true`, while `sidebarLabelColorMetadata=true` proves the metadata still exists without being forced into the visible row chrome.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-side-panel-visual-inventory.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, and `git diff --check` passed. Focused `npm run smoke:ui:auto -- --sidebar` first hit the expected sandbox `listen EPERM 127.0.0.1`; the escalated rerun passed with `sessionRowsTextFirst=true`, `sidebarPinnedRowsTextFirst=true`, `sidebarRowDensityCodexLike=true`, and `sidebarPinActionsConsolidated=true`. Evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779801617818.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779801617818.png`. Refreshed `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` first reported `needs-smoke=10` from sandbox infrastructure failures; the escalated rerun passed with 24 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needs-smoke=0`, and `needs-proof=0`; manifest created at `2026-05-26T13:25:56.260Z`.

Remaining: this closes the current live-screenshot-visible leading-dot mismatch. It does not prove provider pin mutation, deeper provider worktree/projectless adapters, or exact live Codex row spacing across other window sizes.

### 2026-05-26 - Compact Header Profile Badge And Contact Sheet

Codex evidence: live screenshot `/private/tmp/codex-current-screen.png` shows compact top-right header chrome without a wide visible provenance/profile pill. The current Orchestrator screenshots still showed `Automated Ui Smoke profile` occupying the primary titlebar, making the header/panel relationship visibly heavier even after the shared header-band work.

Implemented: changed the isolated profile badge in `Titlebar` to an icon-only 28 px control, keeping the profile name in the tooltip and accessible label. Header smoke now gates `profileBadgeCompact=true`, and the side-panel visual inventory now captures a dedicated `header.png` so the `Header and panel interaction` comparison row can directly own header-specific checks. The header/panel contact sheet now groups live Codex, Orchestrator sidebar, focused header, transcript/header, Workbench right panel, and Terminal bottom panel captures.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-side-panel-visual-inventory.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, and `git diff --check` passed. Focused `npm run smoke:ui:auto -- --header` passed with `profileBadgeCompact=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-header-1779802561267.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-header-1779802561267.png`. The first full comparison exposed a real contract wiring gap: `profileBadgeCompact=true` existed in smoke evidence, but the side-panel inventory had no header capture, so the header/panel row reported `mismatch=1`. After adding `header` to the inventory and contact sheet, `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 25 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needs-smoke=0`, and `needs-proof=0`; manifest created at `2026-05-26T13:46:30.862Z`.

Remaining: this closes the visible wide-profile-pill mismatch and makes header/panel interaction more directly comparable. It does not prove exact live Codex pixel spacing, animation timing, or installed-app replacement.

### 2026-05-26 - Sidebar Footer Collapse Affordance

Codex evidence: live screenshot `/private/tmp/codex-current-screen.png` shows the left sidebar footer has Settings on the left and a small sidebar-collapse affordance on the bottom-right. The previous Orchestrator sidebar footer only exposed the Settings row, so the left-panel footer controls did not match Codex and the left sidebar had no local collapse/expand behavior.

Implemented: added a `sidebar-footer-collapse-toggle` beside the Settings footer row, backed by real left-sidebar collapsed state. Collapsing switches the left panel to a 48 px rail with icon-only New chat, Search, Plugins, Automations, Settings, and Expand controls; expanding restores the 240 px sidebar. The state persists in local storage and the expanded footer remains visible in the normal screenshot.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, and `git diff --check` passed before focused smoke. Focused `npm run smoke:ui:auto -- --sidebar` passed with `sidebarFooterCollapseAffordance=true`, proving 240 px expanded width, 48 px collapsed rail width, and restoration to the expanded sidebar; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779803856358.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779803856358.png`. Refreshed `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 25 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needs-smoke=0`, and `needs-proof=0`; manifest created at `2026-05-26T14:03:25.425Z`.
...
### 2026-05-26 - PDF Page, Zoom, And Invert Controls

Codex evidence: `tmp/codex-app-assets/pdf-preview-panel-CkrOHSbs.js` renders PDF artifacts through the shared artifact header with page navigation, zoom controls, invert colors, annotation, and open/download actions. `tmp/codex-app-assets/artifact-preview-status-D5kFRFQE.js` contains the shared previous/next page and zoom percent controls used by artifact previews, while the PDF panel bundle includes `artifactPdfPreview.invertColors` and `artifactPdfPreview.originalColors`.

Implemented: Orchestrator PDF previews now parse a lightweight page count from local PDFs, render previous/next page controls in the shared artifact header center slot, render zoom out/in controls with a visible percent indicator, expose an invert-colors toolbar control, and update the embedded PDF URL fragment with the selected page and zoom. The invert control applies a real CSS filter to the PDF iframe and exposes active/frame state for verification. The smoke fixture PDF now has two pages so the UI proves disabled/enabled navigation states, page changes from `1/2` to `2/2`, zoom changes from `100%` to `125%`, and invert-colors toggling without losing the selected page/zoom. The shared `ArtifactPreviewHeader` now accepts real center content instead of always reserving an empty center slot.
...
Codex evidence: the header/panel parity rows depend on live Codex screenshot evidence when available, but direct Computer Use inspection of `com.openai.codex` remains blocked. A fresh macOS `screencapture` attempt wrote `/private/tmp/codex-current-screen.png`, but the image was all black. The previous comparison only checked file presence, byte size, and age, so it could treat a blank screenshot as usable visual evidence.

Implemented: `scripts/run-codex-side-panel-comparison.mjs` now inspects PNG evidence for the live Codex screenshot rows. It decodes 8-bit RGB/RGBA PNGs, samples reconstructed pixels after PNG unfiltering, and records nonblank metrics (`nonBlackRatio`, `luminanceStdDev`, and color bucket count). The `Header and panel interaction` and `Right-side Workbench shell` rows now mark `/private/tmp/codex-current-screen.png` as `stale/incomplete` when the screenshot is black instead of counting it as live proof.

Verification: `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run compare:codex-side-panels -- --no-fail` passed. The refreshed comparison report remains `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`, but it now records the live screenshot as `stale/incomplete` with `nonBlack=0`, `lumaStdDev=0`, and `colors=1`; report created at `2026-05-26T16:25:14.342Z`.

Remaining: capture a new nonblank live Codex screenshot before using exact visual/pixel comparison for header/panel polish. Until then, keep UI implementation slices tied to Codex bundle evidence plus Orchestrator smoke/contact-sheet checks rather than claiming live screenshot parity.

### 2026-05-26 - Review/Header Contact-Sheet Coverage

Codex evidence: the user-observed mismatch is not only inside Review or the header independently; it is how the right Review/Workbench panel visually participates in Codex's top shell band with the sidebar, main header, and bottom panel. The existing parity row already tracked this as a shell contract, but its contact sheet only foregrounded the generic Workbench right-panel capture.

Implemented: the `Header and panel interaction` comparison row now includes `review-last-turn` and `review-core` captures in addition to sidebar, focused header, transcript/header, Workbench, and Terminal captures. The generated `tmp/codex-side-panel-comparison/header-panel-contact-sheet.html` now shows Review states directly, and the steering docs call out Review/Workbench header interaction as the active relationship to inspect before closing any panel or titlebar slice.

Verification: `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run compare:codex-side-panels -- --no-fail` passed. The comparison report still shows `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; use `--run-smoke --full` when refreshing screenshots.

Remaining: this makes the mismatch harder to miss in the parity list and artifacts, but it does not itself change the UI. The next product slice should use the updated contact sheet to decide whether the right Review panel, Workbench chrome, header actions, or shared shell spacing needs the concrete UI fix.

### 2026-05-26 - Terminal 280px Bottom-Shell Default

Codex evidence: re-checked the current installed Codex bundle and found the root app-shell bottom panel in `tmp/codex-app-assets/app-shell-Bh-lgoQk.js` owns the visible shell default: it stores `app-shell:bottom-panel-height` with a 280 px fallback, clamps through `Tn`, and passes `defaultSize: 280` to the top-edge resize handle. The earlier 400 px note came from app-shell state evidence that is not the root shell default used by `BottomPanel`.

Implemented: Orchestrator's bottom Terminal now opens and resets to a 280 px total shell: 230 px content plus 50 px shared chrome. The terminal content minimum moves to 110 px so the total minimum matches Codex's 160 px shell minimum, and persisted legacy 260 px and 350 px content defaults migrate to the current 230 px target. Terminal smoke assertions now gate the 280 px total shell target instead of the old 400 px target.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. The focused Terminal smoke passed with `terminalBottomPanelSizeDecomposition=true`, `terminalResizeReset=true`, `terminalResizeKeyboard=true`, `terminalSharedAnimationController=true`, `terminalSharedLayoutController=true`, `terminalHeaderSharedChrome=true`, `terminalContentSpacing=true`, and the transfer/snapshot gates; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-1779813550275.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-1779813550275.png`. The focused Terminal visual smoke passed with `terminalVisualPanel=true`, `terminalPanelMaterialSolid=true`, `terminalBottomPanelSizeDecomposition=true`, `terminalVisualTabs=true`, `terminalVisualToolbar=true`, and `terminalVisualHealthyContent=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-visual-1779813551158.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-visual-1779813551158.png`. The full side-panel comparison passed with 25 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; latest manifest: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-26T16:44:15.809Z`; latest comparison report: `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-26T16:44:16.057Z`.

Remaining: this corrects the bottom-panel default height target and the visual screenshot path. Exact live Codex open/close animation timing still needs nonblank live screenshot/video evidence.
...
### 2026-05-26 - Workbench Start-Edge Tab Close Chrome

Codex evidence: the shared app-shell tab controller still owns right-panel tab state in `tmp/codex-app-assets/app-shell-tab-controller-B2eCi4Le.js`, while the root app-shell tab renderer in `tmp/codex-app-assets/app-shell-Bh-lgoQk.js` renders closable tab controls as hidden start-edge close affordances (`start-0`) with `codex.tabs.closeNamed` labels. Orchestrator's shared `PanelTabStrip` still placed the close control on the right edge and padded content on the right, so Workbench chrome could pass compactness checks while using the wrong close affordance geometry.

Implemented: moved the shared panel tab close affordance to the start edge, gave it an opaque tab-matched background when visible, removed the right-edge hover padding, and added `workbenchPanelTabCloseStartEdge=true` to the focused right-panel smoke plus the Workbench comparison row. This applies through the shared tab primitive rather than a one-off Workbench-only selector.
...
### 2026-05-26 - Spreadsheet And Slides Lightweight Renderers

Codex evidence: `tmp/codex-app-assets/artifact-tab-file-kind-fsu6JKhI.js` maps `.xlsx`/`.xlsm` files to spreadsheet artifacts and `.pptx` files to slides artifacts. `tmp/codex-app-assets/artifact-tab-content.electron-DayvYBGS.js` routes those artifacts through extraction plus `PopcornElectronWorkbookPanel` and `PopcornElectronPresentationPanel`, so the Codex side panel renders workbook/presentation content rather than stopping at a generic unavailable body.

Implemented: Orchestrator now parses local XLSX/PPTX OOXML archives through the existing ZIP reader path in main IPC. XLSX previews extract workbook sheets, shared strings, and worksheet rows into a rendered table preview. PPTX previews extract slide XML text into ordered slide outline cards. The Files panel keeps the shared artifact title/type header and Copy path/Open/Reveal actions, but replaces the previous `Preview unavailable` body for smoke-valid XLSX/PPTX files with real rendered content. The smoke fixtures now generate valid minimal XLSX/PPTX archives, and focused Files smoke gates this with `filesSpreadsheetRenderer=true` and `filesSlidesRenderer=true` in addition to the existing artifact-boundary checks.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the escalated reruns passed with `filesSpreadsheetRenderer=true`, `filesSlidesRenderer=true`, `filesSpreadsheetSlidesArtifactBoundary=true`, and the existing Files artifact checks. Latest evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779818131482.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779818131482.png`. Refreshed full visual inventory passed with 25 captures and manifest created at `2026-05-26T17:53:41.092Z`; the comparison report passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`, created at `2026-05-26T17:56:13.834Z`.

Remaining: this closes the local empty-body gap for spreadsheet/slides artifact previews and moves those surfaces from boundary-only classification to rendered content. It does not implement full Codex workbook grid interaction, slide canvas rendering, editing/annotation hooks, or provider-backed artifact metadata.

### 2026-05-26 - DOCX Page And Zoom Controls

Codex evidence: `tmp/codex-app-assets/docx-preview-panel-BjyRJuYA.js` renders DOCX artifacts through a dedicated `docx-preview-panel` / `codex-docx-preview` surface, strips `.docx` from the visible artifact title, exposes `docx-preview-zoom-trigger`, and uses shared artifact preview page/zoom primitives from `tmp/codex-app-assets/artifact-preview-status-D5kFRFQE.js`.

Implemented: Orchestrator DOCX previews now render previous/next page controls in the shared artifact header center slot, zoom out/in controls with a visible percent indicator in the artifact action cluster, a page-count meta item, and a document page surface instead of one flat paragraph list. The smoke fixture now contains enough DOCX paragraphs to force two local pages, and focused Files smoke gates initial page state, next-page navigation from `1/2` to `2/2`, appendix text on page 2, and zoom from `100%` to `125%` with `filesDocumentPageControls=true`. The side-panel comparison now includes the Codex DOCX preview/zoom bundle terms plus that smoke gate.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the escalated rerun passed with `filesDocumentPageControls=true` plus the existing Files artifact checks. Evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779818606033.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779818606033.png`. A full visual-inventory wrapper refreshed the per-view captures but hung before writing its manifest, so its child processes were stopped and the manifest was rebuilt from the completed per-view logs. The refreshed comparison report passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; manifest created at `2026-05-26T18:12:47.048Z`; report created at `2026-05-26T18:12:50.756Z`.

Remaining: this closes DOCX page/zoom control parity at the local text-preview layer. It does not implement Codex's full DOCX renderer fidelity, pagination/layout engine, comments, download/open-provider metadata, or provider-backed artifact annotations.

### 2026-05-26 - Spreadsheet And Slides Header Controls

Codex evidence: `tmp/codex-app-assets/artifact-tab-content.electron-DayvYBGS.js` routes parsed XLSX/PPTX artifacts into `PopcornElectronWorkbookPanel` and `PopcornElectronPresentationPanel` with `renderHeaderZoomControl`. `tmp/codex-app-assets/PopcornElectronWorkbookPanel-TofX9KRG.js` exposes `data-testid="popcorn-electron-workbook-panel"` and header zoom wiring, while `tmp/codex-app-assets/PopcornElectronPresentationPanel-DNKOkXvp.js` exposes `data-testid="popcorn-electron-presentation-panel"`, `popcorn-presentation-page-navigation`, `popcorn-presentation-zoom-select`, and `popcorn-presentation-thumbnails`.

Implemented: Orchestrator XLSX previews now render one active sheet at a time, with previous/next sheet controls in the artifact header center slot and zoom out/in controls with a percent indicator in the header action cluster. The smoke XLSX fixture now has two sheets so the UI proves sheet navigation from `Smoke data` to `Totals` and zoom from `100%` to `125%`. Orchestrator PPTX previews now render a focused active slide surface, previous/next slide controls, zoom controls, and a thumbnail strip in addition to the outline cards. Focused Files smoke gates these with `filesSpreadsheetControls=true` and `filesSlidesControls=true`, and the side-panel comparison now includes the Codex Popcorn workbook/presentation control terms.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the escalated rerun passed with `filesSpreadsheetControls=true`, `filesSlidesControls=true`, `filesSpreadsheetRenderer=true`, `filesSlidesRenderer=true`, and the existing Files artifact checks. Evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779819702367.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779819702367.png`. `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with a refreshed 25-capture manifest and `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; manifest created at `2026-05-26T18:27:37.894Z`; report created at `2026-05-26T18:27:38.157Z`.

Remaining: this closes the XLSX/PPTX control-layer gap for local previews. It does not implement full Popcorn workbook grid interaction, slide canvas fidelity, speaker notes, editable annotations, comments, or provider-backed artifact metadata.

### 2026-05-26 - Spreadsheet Sheet Tab Bar

Codex evidence: `tmp/codex-app-assets/PopcornElectronWorkbookPanel-TofX9KRG.js` exposes workbook page chrome through `popcorn-pages-bar`, `popcorn-sheet-tab-*`, `popcorn-add-sheet`, and `popcorn-zoom-select`, so sheet switching is represented as a workbook sheet-tab bar rather than only header previous/next controls.

Implemented: Orchestrator XLSX previews now render a bottom sheet-tab bar for parsed local workbook sheets. The active sheet tab is marked with `aria-selected`, clicking another tab switches the visible sheet, and the add-sheet affordance is present but disabled because the current Files artifact preview is read-only and does not implement workbook editing.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `filesSpreadsheetSheetTabs=true`, `filesSpreadsheetControls=true`, and `filesSpreadsheetRenderer=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779843784918.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779843784918.png`. The full visual inventory passed with 26 captures and no failures, including `filesSpreadsheetSheetTabs=true` in the Files capture; latest Files evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779843901226.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779843901226.png`; manifest created at `2026-05-27T01:08:34.204Z`. `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-27T01:08:40.223Z`.

Remaining: this closes the visible workbook sheet-tab mismatch for local read-only XLSX previews. Full Popcorn workbook grid interaction, editable sheet creation, formulas, comments, annotations, and provider-backed artifact metadata remain open.

### 2026-05-26 - Spreadsheet Active Cell Formula Bar

Codex evidence: `tmp/codex-app-assets/PopcornElectronWorkbookPanel-TofX9KRG.js` exposes `popcorn-formula-bar` and `popcorn-selection` alongside the existing workbook sheet-tab and zoom primitives, so the workbook surface is not just a passive table.

Implemented: Orchestrator XLSX previews now render workbook-style column and row headers, selectable read-only cells, active-cell metadata on the preview root and formula bar, and a read-only formula/value field. Selecting cells updates the active address/value, and switching sheets resets the active cell to the new sheet's top-left cell instead of carrying stale state across sheets. Focused Files smoke now gates this with `filesSpreadsheetActiveCell=true`, and the side-panel comparison requires that gate for the Files/file-source row.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `filesSpreadsheetActiveCell=true`, `filesSpreadsheetSheetTabs=true`, and `filesSpreadsheetControls=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779844791812.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779844791812.png`. The full visual inventory passed with 26 captures and no failures, including `filesSpreadsheetActiveCell=true` in the Files capture; latest Files evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779844922268.json`; screenshot: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/files.png`; manifest created at `2026-05-27T01:25:35.975Z`. `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-27T01:25:39.063Z`.
...
Implemented: promoted the user-observed header/panel relationship concern into the current top-level parity language for Files, Browser, Review, Workbench, the main titlebar, the left sidebar, and the bottom Terminal panel. The header/panel contact sheet contract now also includes the `files` and `browser` captures, so the comparison artifact can catch relationship drift in those right-panel states instead of only Workbench and Review. The latest full side-panel comparison evidence is now reflected in the steering matrix and live-comparison doc instead of the older `17:19` run, so future slices must preserve the refreshed `18:27` header/panel row before being treated as parity-safe.

Verification: `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run compare:codex-side-panels -- --no-fail` passed. The regenerated comparison report from `2026-05-26T18:34:22.316Z` reported `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0` against the 25-capture visual inventory created at `2026-05-26T18:27:37.894Z`. The regenerated `tmp/codex-side-panel-comparison/header-panel-contact-sheet.html` now lists Files and Browser captures in the header/panel review aid. No app source behavior changed in this comparison-contract refresh.

Remaining: exact live Codex pixel spacing and animation timing for the coordinated header/panel relationship still require fresh nonblank live screenshot or video evidence.

### 2026-05-26 - Workbench Transfer Comparison Gates

Codex evidence: `app-shell-tab-controller-B2eCi4Le.js` exposes shared `moveTabTo` / `receiveMovedTab` controller semantics, while `thread-page-bottom-panel-state-D1Lz0U4Y.js` remains terminal-specific for the current bottom-panel renderer. That means terminal transfer belongs in the parity matrix, and non-terminal right-to-bottom transfer should stay explicit as an unsupported boundary until there is product-backed Codex evidence for bottom renderers beyond Terminal.

Implemented: the right-panel Terminal context action now uses generic shell wording, `Move tab to bottom panel`, matching the bottom-panel `Move tab to right panel` style instead of special-casing the label as terminal-only. The full side-panel inventory now includes a separate `terminal-behavior` capture backed by `--terminal`, so the comparison can require `terminalSharedTransferModel=true`, `terminalMoveToRightPanel=true`, and `terminalMoveBackToBottom=true` without overloading the stable visual `terminal-bottom-panel` screenshot. The Workbench comparison row also now requires `rightPanelTransferUnsupportedBoundary=true`, so Review/Files/Browser unsupported transfer metadata is part of the direct parity report.

Verification: `node -c scripts/run-side-panel-visual-inventory.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, and `pnpm exec tsc --noEmit` passed. The sandboxed focused smokes first hit the expected `listen EPERM 127.0.0.1` infrastructure failure; elevated reruns passed. Focused Terminal smoke passed with `terminalSharedTransferModel=true`, `terminalMoveToRightPanel=true`, and `terminalMoveBackToBottom=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-1779822058884.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-terminal-1779822058884.png`. Focused right-panel smoke passed with `rightPanelTransferUnsupportedBoundary=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779822056924.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779822056924.png`. The direct full visual inventory wrapper passed with 26 captures and no failures, manifest created at `2026-05-26T19:06:50.755Z`; `npm run compare:codex-side-panels -- --no-fail` then passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-26T19:06:53.979Z`.

Remaining: exact live Codex pixel spacing and animation timing still require fresh nonblank live screenshot or video evidence. Non-terminal bottom-panel renderers remain a product/runtime decision, not a parity assumption, because the current Codex bottom-panel bundle is terminal-specific.

### 2026-05-26 - Files And Browser Header Seam Gates

Codex evidence: `app-shell-state-HP0T5lEX.js` and `thread-page-bottom-panel-state-D1Lz0U4Y.js` keep the right and bottom panels under shared app-shell ownership, while the live screenshot route remains stale/incomplete for exact pixel proof. The user-observed mismatch risk is specifically about how panels meet the header, so Files and Browser need executable seam evidence, not only screenshots in the contact sheet.

Implemented: focused Files smoke now emits `filesHeaderPanelSeam=true` when the Files right-panel state shares the same top seam as the session titlebar, keeps the Workbench chrome in the header band, and places the Files toolbar directly below that chrome. Focused Browser smoke now emits `browserHeaderPanelSeam=true` for the same titlebar/right-panel/chrome relationship in the Browser state. The side-panel comparison requires both checks in the dedicated `Header and panel interaction` row, so Files and Browser can no longer pass only as screenshots in the contact sheet.

Verification: `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `pnpm exec tsc --noEmit`, `git diff --check`, and `npm run build` passed. Focused Files smoke passed with `filesHeaderPanelSeam=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779820982611.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779820982611.png`. Focused Browser smoke passed with `browserHeaderPanelSeam=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-browser-1779821018113.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-browser-1779821018113.png`. The direct full visual inventory wrapper passed with 25 captures and no failures, manifest created at `2026-05-26T18:53:31.947Z`; `npm run compare:codex-side-panels -- --no-fail` then passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-26T18:53:37.188Z`.

Remaining: exact live Codex pixel spacing and animation timing still require fresh nonblank live screenshot or video evidence. The full comparison wrapper may still classify all rows as `needs-smoke` when run without the localhost-capable path; use direct elevated visual inventory plus `compare --no-fail` if that happens.

Recommended order:

1. Shell/tab foundation and panel/header interaction first: keep the dedicated comparison row green whenever changing the left sidebar, main titlebar, Workbench right panel, Files/Review/Browser Workbench chrome, or Terminal bottom panel, and review `tmp/codex-side-panel-comparison/header-panel-contact-sheet.html` because the user-visible mismatch can be in the relationship between surfaces rather than in any one panel. Header action controls must stay compact toolbar chrome inside that shared band, not a heavier independent button cluster.
...
4. Terminal live Codex bottom-panel height/animation comparison at multiple window sizes plus root shell ownership for any remaining non-shared tab paths. The local content/chrome/target decomposition is now smoke-gated at a Codex-bundle-aligned 280 px total default/reset target; exact live animation timing remains.
...
### 2026-05-26 - Notebook Raw Output Disclosure And MIME Precedence

Codex evidence: `tmp/codex-app-assets/notebook-preview-panel-CAO-aRhM.js` renders summarized notebook raw payloads behind a `Raw output` disclosure and chooses `text/plain` before `application/json` / `application/vnd.vega.v5+json` when both MIME payloads are present in a display output.

Implemented: Orchestrator notebook previews now use a Codex-style `Raw output` disclosure for summarized stream/text/JSON/error payloads and HTML raw source, while leaving unsummarized raw payloads directly visible. The notebook output parser now follows Codex MIME precedence by rendering `text/plain` before JSON when both are present. The Files smoke fixture now includes both MIME values and proves the rendered output is text/plain, not JSON.

Verification: `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `pnpm exec tsc --noEmit`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `filesNotebookRawOutputDisclosure=true`, `filesNotebookOutputSummaries=true`, and `filesNotebookOutputRendering=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779823093321.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779823093321.png`. A direct full visual-inventory wrapper refreshed all 26 captures with no failures, manifest created at `2026-05-26T19:26:07.741Z`; `npm run compare:codex-side-panels -- --no-fail` then passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-26T19:26:15.363Z`.

Remaining: this closes the concrete raw-output/MIME behavior mismatch. Exact notebook cell spacing is now covered by the later spacing slice; full provider-backed artifact metadata remains open.

### 2026-05-26 - Notebook Code Snippet Shell

Codex evidence: `tmp/codex-app-assets/notebook-preview-panel-CAO-aRhM.js` renders notebook code-cell source through the shared code-snippet path with a `Python` title, including when source is hidden behind a described-cell `Code` disclosure.

Implemented: Orchestrator notebook previews now render code-cell source in a compact titled code shell instead of a bare `<pre>`. Described code cells still keep source hidden behind the existing read-only `Code` disclosure, but the disclosed source now has the Codex-like `Python` title. Focused Files smoke gates this with `filesNotebookCodeSnippet=true`.

Verification: `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `pnpm exec tsc --noEmit`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `filesNotebookCodeSnippet=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779823811213.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779823811213.png`. The full visual inventory passed with 26 captures and no failures, manifest created at `2026-05-26T19:36:04.104Z`; `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-26T19:36:10.578Z`.

Remaining: this aligns the titled code-source shell. Exact notebook cell spacing is now covered by the later spacing slice; full provider-backed artifact metadata remains open.

### 2026-05-26 - Notebook Cell Spacing And Width

Codex evidence: `tmp/codex-app-assets/notebook-preview-panel-CAO-aRhM.js` wraps notebook cells in `px-4 py-4 sm:px-6 sm:py-5`, centers the cell column with `mx-auto`, caps it at `max-w-3xl`, uses `gap-4`, and renders `rounded-lg` cells with `px-4 py-2` headers.

Implemented: Orchestrator notebook previews now use a centered `notebook-preview-list-inner` column with 768 px max width, 16 px cell gap, 16 px mobile padding, 20 px/24 px wider padding, 8 px cell radius, and 8 px/16 px summary headers. Focused Files smoke gates this with `filesNotebookCellSpacing=true`.

Verification: `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `pnpm exec tsc --noEmit`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `filesNotebookCellSpacing=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779834968831.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779834968831.png`. The full visual inventory passed with 26 captures and no failures, manifest created at `2026-05-26T22:41:47.977Z`; `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-26T22:41:59.925Z`.

Remaining: this closes the exact notebook cell spacing/width mismatch. Deeper output renderer polish and full provider-backed artifact metadata remain open.

### 2026-05-26 - Notebook Output And Source Chrome

Codex evidence: `tmp/codex-app-assets/notebook-preview-panel-CAO-aRhM.js` renders notebook cell source bodies with `px-4 py-3`, and renders code-cell outputs in a separate `border-t` secondary-tinted band with `px-4 py-3`, an inner `flex flex-col gap-3` stack, and an inset top highlight.

Implemented: Orchestrator notebook previews now wrap source content in a padded source body and render outputs in a separate bordered, lightly tinted output band with 16 px horizontal padding, 12 px vertical padding, 12 px output gap, and an inset top highlight. Focused Files smoke gates this with `filesNotebookOutputChrome=true`. The visual-inventory wrapper also now time-boxes individual smoke captures and accepts completed output JSON plus copied screenshots with all checks passing, preventing a child shutdown hang from blocking the comparable manifest.

Verification: `node -c scripts/run-side-panel-visual-inventory.mjs`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `pnpm exec tsc --noEmit`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `filesNotebookOutputChrome=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779835804010.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779835804010.png`. The refreshed full visual inventory passed with 26 captures and no failures, including `filesNotebookOutputChrome=true` in the Files capture; latest Files evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779836180093.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779836180093.png`; manifest created at `2026-05-26T22:59:57.583Z`. `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-26T23:00:10.516Z`.

Remaining: this closes the notebook source/output band chrome mismatch. Deeper per-output item styling and full provider-backed artifact metadata remain open.

### 2026-05-26 - Notebook Stream/Text Output Item Chrome

Codex evidence: `tmp/codex-app-assets/notebook-preview-panel-CAO-aRhM.js` renders summarized stream/text outputs through rounded `bg-token-main-surface-primary/40 p-3` item containers and raw payloads through rounded code-block surfaces with `p-3`. The same bundle also shows separate markdown/image/error item treatments, but the current smoke fixture only exercises stream/text outputs reliably.

Implemented: Orchestrator notebook stream/text output items now use Codex-like rounded, lightly tinted item containers with 12 px padding. Raw output pre blocks now use rounded, lightly tinted code surfaces with 12 px padding. Markdown and error output CSS was also adjusted toward the Codex bundle shapes, but separate markdown/error/image fixture proof remains open because expanding the current notebook fixture to those output types caused the focused Files smoke to time out before writing output JSON.

Verification: `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `pnpm exec tsc --noEmit`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `filesNotebookOutputItemChrome=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779837123625.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779837123625.png`. The full visual inventory passed with 26 captures and no failures, including `filesNotebookOutputItemChrome=true` in the Files capture; latest Files evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779837249250.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779837249250.png`; manifest created at `2026-05-26T23:17:57.522Z`. `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-26T23:20:49.094Z`.

Remaining: this closes stream/text output item chrome for the current notebook fixture. Separate markdown/error/image item fixture coverage and full provider-backed artifact metadata remain open.

### 2026-05-26 - Notebook Rich Output Item Chrome Fixture

Codex evidence: `tmp/codex-app-assets/notebook-preview-panel-CAO-aRhM.js` renders markdown and image outputs as contained item surfaces and error outputs as red-tinted, bordered `p-3` containers. The prior stream/text item-chrome slice already aligned the shared output container CSS but only smoke-proved the stable stream/text fixture.

Implemented: added a separate `notebook-output-types-smoke.ipynb` fixture instead of expanding the existing stream/text notebook fixture. The new fixture exercises markdown, error, and image notebook outputs and gates their item chrome with `filesNotebookRichOutputItemChrome=true`. The side-panel comparison now requires that flag in the Files row. The visual-inventory runner now launches each smoke capture directly with a process-group timeout, which fixed the full-inventory hang that previously stopped in `diff-core` before the wrapper could return comparable evidence.

Verification: `node -c scripts/run-side-panel-visual-inventory.mjs`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `pnpm exec tsc --noEmit`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` passed with `filesNotebookRichOutputItemChrome=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779837960139.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779837960139.png`. The refreshed full visual inventory passed with 26 captures and no failures, including `filesNotebookRichOutputItemChrome=true` in the Files capture; latest Files evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779838398949.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779838398949.png`; manifest created at `2026-05-26T23:36:53.060Z`. `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-26T23:40:07.774Z`.

Remaining: this closes separate markdown/error/image notebook item fixture coverage. Provider-backed artifact metadata and full artifact renderer fidelity remain open.

### 2026-05-26 - Artifact Open Options Header Controls

Codex evidence: `tmp/codex-app-assets/artifact-preview-status-D5kFRFQE.js` renders artifact header actions around a primary Open affordance with adjacent open options/reveal behavior, rather than exposing Open and Reveal as equal loose toolbar buttons.

Implemented: Orchestrator artifact preview headers now group local open behavior behind a primary `Open` toolbar button plus an adjacent `Open options` chevron/menu. JSON, CSV, DOCX, PDF, XLSX, PPTX, and IPYNB previews all expose `data-artifact-open-options="true"` and the menu contains `Open` and `Open in folder`. The DOCX, PDF, spreadsheet, and slides custom headers keep their page/zoom controls while sharing the same Open/options affordance.

Verification: `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `pnpm exec tsc --noEmit`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `filesArtifactPreviewControls=true` and `filesArtifactOpenOptions=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779840151421.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779840151421.png`. The full visual inventory passed with 26 captures and no failures, including `filesArtifactOpenOptions=true` in the Files capture; latest Files evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779840298342.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779840298342.png`; manifest created at `2026-05-27T00:08:31.179Z`. `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-27T00:08:34.265Z`.

Remaining: local artifact action chrome now matches the Codex-shaped Open/options control. Remote download/open-provider menu items, full artifact renderer fidelity, and provider-backed artifact metadata remain open.

### 2026-05-26 - Workbench File-Source Action Labels

Codex evidence: `tmp/codex-app-assets/review-file-source-tab-CjS7Xe_W.js` exposes `File viewer options` for the file-source options menu, `Open in editor` for the open target, `Copy path`, `Enable/Disable rich view`, `Enable/Disable word wrap`, and `Show/Hide git blame`.

Implemented: Orchestrator file-source tabs now use `File viewer options` for the ellipsis button label/tooltip, `Open in editor` for file-open actions, `Open selected line in editor` for selected-line actions, and `Enable/Disable word wrap` in both the toolbar and the shared options menu. The existing shared action-menu sections remain intact, and the smoke now rejects the older `source wrap` copy.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `workbenchFileTabCodexActionLabels=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779840863976.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779840863976.png`. The full visual inventory passed with 26 captures and no failures, including `workbenchFileTabCodexActionLabels=true` in the Files capture; latest Files evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779841007731.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779841007731.png`; manifest created at `2026-05-27T00:20:20.371Z`. `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-27T00:20:24.298Z`.

Remaining: this closes the visible file-source action-label mismatch. Deeper structural source-tab toolbar/menu parity, provider-backed comments/blame, provider/global indexed search, and full artifact renderer fidelity remain open.

### 2026-05-26 - Workbench File-Source Action Cluster

Codex evidence: `tmp/codex-app-assets/review-file-source-tab-CjS7Xe_W.js` renders the file-source breadcrumb/action cluster as the `File viewer options` menu (`Vr`) plus the `Open in editor` button (`Br`). Rich/source view, word wrap, copy path, artifact preview, and git blame live behind the options menu rather than as a large row of visible toolbar buttons.

Implemented: Orchestrator file-source tabs now keep source/rich, word wrap, git blame, copy path, reveal, add-to-chat, and selected-line utilities in the existing options menu or selected-line affordances, while the visible file-tab action cluster is only `File viewer options` followed by `Open in editor`. The source-mode and wrap toolbar controls remain mounted for state/test continuity but are hidden with the existing secondary-action class, so the visible action cluster matches the Codex bundle structure.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `workbenchFileTabCodexActionCluster=true` and the existing Files gates; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779841776063.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779841776063.png`. The full visual inventory passed with 26 captures and no failures, including `workbenchFileTabCodexActionCluster=true` in the Files capture; latest Files evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779842062956.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779842062956.png`; manifest created at `2026-05-27T00:37:54.902Z`. `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-27T00:38:02.171Z`.

Remaining: this closes the visible file-source action-cluster mismatch. Source-search/global-find structure, provider-backed comments/blame, provider/global indexed search, and full artifact renderer fidelity remain open.

### 2026-05-26 - Workbench File-Source Shared Find

Codex evidence: `tmp/codex-app-assets/review-file-source-tab-CjS7Xe_W.js` registers source search through `sourceSearchSource`, and `tmp/codex-app-assets/review-runtime-bridge-CZUIqW4U.js` owns the shared `content-search-input` with `Search chat` / `Search diffs` scope controls.

Implemented: Orchestrator file-source tabs no longer render the inline `workbench-file-tab-source-search` toolbar input. When a focused source file tab receives `Cmd+F`, Orchestrator now opens the shared thread find UI in diff scope and routes shared query, next/previous, and close events into file-source line highlights, active-match metadata, and source-search clearing.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `fileSourceSearch=true`, no inline source search field, shared `content-search-input` focused in diff scope, source line highlights, and next-result stepping; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779842770987.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779842770987.png`. The full visual inventory passed with 26 captures and no failures, including `fileSourceSearch=true`, `workbenchFileTabCodexActionLabels=true`, and `workbenchFileTabCodexActionCluster=true` in the Files capture; latest Files evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779842940963.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779842940963.png`; manifest created at `2026-05-27T00:52:33.639Z`. `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-27T00:52:37.405Z`.

Remaining: this closes the visible inline source-search mismatch. Exact shared-find focus timing, provider/global indexed workspace search, provider-backed comments/blame, and full artifact renderer fidelity remain open.

### 2026-05-27 - Installed Side-Panel Comparison Proof

Codex evidence: the current side-panel comparison still uses the installed Codex bundle and the live screenshot route only when it produces nonblank pixels. Live pixel/timing proof remains unavailable in this environment because the current macOS capture route returns black images, but the Orchestrator installed-app path itself is now directly comparable to the dev and packaged paths.

Implemented: fixed installed-app smoke reliability by waiting briefly for late smoke output after the installed Electron launcher exits, keeping the browser smoke server alive until the report is read, and avoiding a destroyed `webContents` access in the window `closed` handler. The installed smoke path now supports the real `/Applications/Orchestrator.app` bundle without turning a passing UI smoke into a failed process.

Verification: `npm run pack:mac` passed, then `npm run install:mac` installed the rebuilt app to `/Applications/Orchestrator.app`. Elevated focused `npm run smoke:ui:auto -- --installed --sidebar` passed with `hoverCardMaterial=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779849160385.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-sidebar-1779849160385.png`. Elevated full installed comparison `npm run compare:codex-side-panels -- --run-smoke --full --installed --no-fail --smoke-out tmp/side-panel-visual-inventory-installed-full` passed with 27 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; manifest: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-installed-full/manifest.json`, created at `2026-05-27T02:39:27.251Z`; comparison report: `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T02:39:27.500Z`.

Remaining: this closes the installed-app replacement proof gap for the current side-panel matrix. Exact live Codex pixel spacing, OS focus behavior, and animation timing still require a fresh nonblank live Codex screenshot or video path.

### 2026-05-27 - App-Shell Panel Open/Close Telemetry

Codex evidence: `tmp/codex-app-assets/app-shell-tab-controller-B2eCi4Le.js` emits `codex_thread_side_panel_opened`, `codex_thread_side_panel_closed`, and `codex_thread_side_panel_tab_viewed`, with active-tab and route-kind metadata. That means panel open/close lifecycle belongs to the shared app shell, not only to individual tab views.

Implemented: shared `AppShellPanel` now records `panel.opened` and `panel.closed` through `window.api.performance.record`, including `panelId`, `panelSurface`, `activeTab`, and `routeKind`. Workbench right panel and Terminal bottom panel both pass their active tab and `local_thread` route kind into the shared shell primitive. Focused right-panel smoke now verifies both `panel.closed` and `panel.opened` for the right Workbench surface, and the Codex side-panel comparison requires `rightPanelPanelOpenCloseTelemetry=true` in the Right-side Workbench shell row.

Verification: `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, and `pnpm exec tsc --noEmit` passed. Focused `npm run smoke:ui:auto -- --right-panel` passed after the expected sandbox `listen EPERM 127.0.0.1` on the unelevated attempt; evidence JSON `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779850119191.json`; screenshot `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-right-panel-1779850119191.png`. `npm run pack:mac` and `npm run install:mac` passed, then elevated full installed comparison `npm run compare:codex-side-panels -- --run-smoke --full --installed --no-fail --smoke-out tmp/side-panel-visual-inventory-installed-full` passed with 27 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; manifest `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-installed-full/manifest.json`, created at `2026-05-27T02:53:31.332Z`; comparison report `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T02:53:31.584Z`.

Remaining: this narrows the route/window lifecycle gap by closing the product-level side-panel open/close telemetry piece. Broader route/window lifecycle ownership, exact live Codex pixel spacing, OS focus behavior, and panel animation timing remain open.

### 2026-05-27 - Window-Scoped Pending Navigation

Codex evidence: Codex uses route-backed thread identity and shell/window lifecycle routing rather than a single app-global pending route. The Orchestrator audit already tracked route/window lifecycle as a shell gap because `Open in new window` depended on pending navigation during renderer boot.

Implemented: replaced the app-global pending-navigation handoff for new session windows with a `webContents.id` scoped pending-navigation map. Deep links still use a fallback pending navigation when no target window exists, but focused/deep-link targets and newly created session windows now receive their own pending route. The multi-window smoke intentionally asks the opener to consume pending navigation after opening the second window; that would steal the new window's route with the previous global slot, and now passes with `pendingNavigationWindowScoped=true` while the second window still navigates to the requested session.

Verification: `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, and `pnpm exec tsc --noEmit` passed. Focused `npm run smoke:ui:auto -- --multi-window-focus` passed with `pendingNavigationWindowScoped=true`; evidence JSON `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-multi-window-focus-1779851098320.json`; screenshot `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-multi-window-focus-1779851098320.png`. Elevated full `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 27 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; manifest `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-27T03:10:13.279Z`; comparison report `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T03:10:13.525Z`. The full comparison's multi-window capture evidence JSON is `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-multi-window-focus-1779851347911.json`.

Remaining: this closes one concrete global-route handoff weakness. Broader route/window lifecycle ownership, exact live Codex pixel spacing, OS focus behavior, and panel animation timing remain open.

### 2026-05-27 - Loaded-Window Deep-Link Pending Cleanup

Codex evidence: Codex treats route-backed thread identity as shell/window lifecycle state. The previous Orchestrator route-window slice closed the new-window global pending-route leak, but a focused deep link into an already-loaded Orchestrator window still wrote a scoped pending-navigation entry even though the renderer also received the direct navigation event.

Implemented: `navigateFromDeeplink` now only stores scoped pending navigation for target windows that are still loading. Already-loaded windows receive the direct app navigation event without retaining stale pending state, and the multi-window smoke proves that a loaded-window session deep link does not make a later `consumePendingNavigation()` return an already-handled route. The header/panel comparison row now requires `loadedDeepLinkDoesNotLeavePendingNavigation=true`.
...
Codex evidence: the installed-app comparison remains the release-level proof that Orchestrator's side-panel matrix is runnable outside dev mode. Because the previous slice changed main-process route handoff behavior, `/Applications/Orchestrator.app` needed a fresh installed comparison rather than relying on the earlier installed matrix.

Implemented: rebuilt the packaged app with `npm run pack:mac`, installed it with `npm run install:mac`, and refreshed the full installed side-panel comparison against `/Applications/Orchestrator.app`. No app source changes were required in this slice; the work updates the parity ledger to point at the current installed evidence after the loaded-window deep-link fix.

Verification: `npm run pack:mac` passed and rebuilt `dist/mac-arm64/Orchestrator.app`; `npm run install:mac` installed it to `/Applications/Orchestrator.app`. Elevated full installed comparison `npm run compare:codex-side-panels -- --run-smoke --full --installed --no-fail --smoke-out tmp/side-panel-visual-inventory-installed-full` passed with 27 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; installed manifest `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-installed-full/manifest.json`, created at `2026-05-27T03:37:08.348Z`; comparison report `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T03:41:51.077Z`. The installed multi-window evidence JSON is `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-multi-window-focus-1779852980419.json` and includes `loadedDeepLinkDoesNotLeavePendingNavigation=true`.

Remaining: installed replacement proof is current for the side-panel matrix after the route lifecycle fix. Exact live Codex pixel spacing, OS focus behavior, and panel animation timing still require nonblank live Codex screenshot or video evidence.

### 2026-05-27 - Optional Live Screenshot Evidence Status

Codex evidence: the current `/private/tmp/codex-current-screen.png` live screenshot path still exists but is black, with `nonBlack=0`, `lumaStdDev=0`, and `colors=1`. The markdown comparison already displayed that file as `stale/incomplete`, but the JSON aggregate for optional file evidence could still report `file.passed=true` because the file existed.

Implemented: `scripts/run-codex-side-panel-comparison.mjs` now reports aggregate file evidence `passed=false` whenever any file evidence fails its own byte/freshness/nonblank checks, including optional live screenshots. Optional file evidence still does not turn a smoke-backed row into a mismatch unless the contract marks it required, so this corrects evidence truthfulness without blocking fixture-covered rows while live capture is unavailable.

Verification: `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `pnpm exec tsc --noEmit` passed. `npm run compare:codex-side-panels -- --installed --smoke-out tmp/side-panel-visual-inventory-installed-full --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T03:41:51.077Z`, keeps the Header/panel and Workbench rows `fixture-covered` while reporting their optional live screenshot file evidence as `passed=false`.
...
Implemented: added a host-scoped Orchestrator `Browser` Settings section using the shared Settings page/content/surface/row primitives. The page clears real in-app Browser data for the selected local host partition through the existing Electron `browser:clearData` path, with individual rows for all browsing data, cookies, site data, and cached images/files. A follow-up slice added durable Browser Use approval/history/download/upload/domain defaults, so the page now edits real persisted policy instead of merely pointing users to session-scoped Security controls.

Verification: `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, `pnpm exec tsc --noEmit`, and `npm run build` passed. Focused Settings smoke first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `settingsBrowserPage=true`, `settingsBrowserSurface=true`, `settingsBrowserModule=true`, and `settingsContentLayout=true`; evidence JSON `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-1779854070171.json`; screenshot `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-1779854070171.png`. Elevated full `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 27 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; manifest `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-27T04:01:31.756Z`; comparison report `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T04:01:32.014Z`. The Settings comparison row now requires `settingsBrowserPage`, `settingsBrowserSurface`, and `settingsBrowserModule`.

Remaining: this closes the missing Settings Browser page/data-control gap. The persisted policy path is covered in the next entry. Remote-host Browser settings adapters, provider-backed Browser Use runtime state, and live Codex Settings screenshot comparison remain open.

### 2026-05-27 - Browser Use Policy Persistence

Codex evidence: `tmp/codex-app-assets/browser-use-settings-DMAxxI2Y.js` contains approval/history copy and domain policy list surfaces in Browser Use settings, while the Browser sidebar bundle keeps approval/domain behavior visible in the Browser side-panel Security surface.

Implemented: added `browserUsePolicy` to persisted app settings with shared normalization/default helpers. Settings > Browser now edits approval, history, download, upload, and domain allow/block defaults through real persisted settings. New Browser workbenches adopt those defaults when their policy is still at the app default, without stomping per-session Security pane overrides.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused Settings smoke passed with `settingsBrowserPolicyPersistence=true`; evidence JSON `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-1779855428419.json`; screenshot `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-settings-1779855428419.png`. Focused Browser smoke passed with `browserPersistedPolicyDefaults=true`; evidence JSON `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-browser-1779855428672.json`; screenshot `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-browser-1779855428672.png`. Elevated full `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 27 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; manifest `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-27T04:22:34.215Z`; comparison report `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T04:22:34.470Z`.

Remaining: this closes the durable Browser Use approval/history/download/upload/domain default gap for local Browser workbenches. Provider-backed Browser Use adapters, remote-host policy state, broader click/type/screenshot browser-use requests, and live Codex Settings screenshot comparison remain open.

### 2026-05-27 - Review Provider Source Unavailable Reasons

Codex evidence: Codex Review exposes provider-native source concepts such as Last turn and hosted/cloud/worktree Review sources, but Orchestrator only has real local/session-backed source data plus normalized provider diff events when the provider actually emits them. The current live Codex app-server proof has provider session/turn ids but no working-tree checkpoint restore and no cloud/provider-hosted diff adapter.

Implemented: unsupported provider-native Review source rows now stay visible but explain their unavailable boundary inline and through smoke-visible attributes. Last turn reports `No provider turn diff` until a provider diff event exists, Cloud reports `Cloud review adapter missing`, Local reports `No local provider source` when a session cannot supply local review data, and Worktree reports `No provider worktree source` unless the session is app-managed worktree-backed. Disabled rows also expose the reason in the button title and ARIA label, while supported rows keep their compact count badge.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --diff-source` passed with `reviewProviderSourceUnavailableReasons=true` plus the existing source gates; evidence JSON `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-source-1779856661582.json`; screenshot `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-diff-source-1779856661582.png`. Elevated full `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 27 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; manifest `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-27T04:48:16.251Z`; comparison report `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T04:48:16.500Z`.

Remaining: this closes the explicit unsupported-boundary UI gap. It does not implement cloud/hosted Review adapters, provider checkpoint restore, live commented-PR proof, provider-native blame/comments, full diff virtualization, or exact Review pixel spacing.

### 2026-05-27 - Browser Dynamic Click/Type Tools

Codex evidence: the Codex app-server bundle exposes `dynamic-tools-for-thread-start-requested`, `dynamic-tool-call-requested`, and `item/tool/call`, while the Browser bundle still contains browser-use route/capture state. The prior live proof showed Codex will request advertised Orchestrator Browser dynamic tools for open/read, so this slice extends that same proven provider path rather than inventing a fixture-only Browser control.

Implemented: Orchestrator now advertises `orchestrator.browser_click` and `orchestrator.browser_type` alongside `browser_open` and `browser_read` for Codex app-server sessions. Browser open/read responses include visible target metadata with node ids, target previews, and primary selectors. Click/type requests route through the existing `browser:clientToolCall` IPC bridge into the Browser renderer, resolve a target by node id, selector, visible text, target text, or index, execute the action in the actual webview, and return a post-action Browser snapshot plus target action result.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `node -c scripts/codex-browser-appserver-live-proof.mjs`, `git diff --check`, `npm run test:providers` (293/293), and `npm run build` passed. Focused `npm run smoke:ui:auto -- --browser` passed with `browserClientToolBridge=true` and `browserClientToolActions=true`; final evidence JSON `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-browser-1779858338992.json`; screenshot `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-browser-1779858338992.png`. Live `npm run live:codex-browser-tools` passed with real Codex app-server `item/tool/call` requests for `browser_open`, `browser_read`, `browser_click`, `browser_type`, and a final `browser_read`; artifact `/Users/nadav/Desktop/Orchestrator/tmp/codex-browser-tools-live-proof/result.json`, created at `2026-05-27T04:56:24.017Z`. Elevated full `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` passed with 27 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; manifest `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-27T05:11:06.568Z`; comparison report `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T05:11:06.831Z`.

Remaining: this closes the real-provider open/read/click/type Browser dynamic-tool gap. Native browser-use event streaming, screenshot/image-return tooling, richer pointer/keyboard actions, provider design-change application, and installed-app end-to-end Browser agent proof remain separate Browser backlog items.

### 2026-05-27 - Browser Dynamic Screenshot Tool

Codex evidence: the prior real Browser dynamic-tool proof established that the live Codex app-server will request advertised production Browser tools through `item/tool/call`. The Browser bundle also tracks browser-use capture-surface state, so screenshot/capture coverage is the next concrete Browser-agent parity step that can be proven through the same real app-server path.

Implemented: Orchestrator now advertises `orchestrator.browser_screenshot` alongside `browser_open`, `browser_read`, `browser_click`, and `browser_type` for Codex app-server sessions. Screenshot requests route through the existing `browser:clientToolCall` IPC bridge into `BrowserPanel`, capture the active webview with Electron `capturePage()`, save the PNG through the existing Browser artifact path, open the Browser inspector surface, and return URL/title/visible-structure/targets plus screenshot metadata including MIME type, dimensions, byte size, and artifact path.

Verification: `pnpm exec tsc --noEmit`, `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `node -c scripts/codex-browser-appserver-live-proof.mjs`, `npm run test:providers` (293/293), and `npm run build` passed. Focused `node scripts/run-automated-ui-smoke.mjs --browser` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `browserClientToolBridge=true`, `browserClientToolActions=true`, and `browserClientToolScreenshot=true`; evidence JSON `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-browser-1779859073968.json`; screenshot `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-browser-1779859073968.png`. Live `npm run live:codex-browser-tools` first hit the expected sandbox boundary reading `/Users/nadav/.codex/sessions`; the elevated rerun passed with real Codex app-server `item/tool/call` requests for `browser_open`, `browser_read`, `browser_click`, `browser_type`, `browser_screenshot`, and a final `browser_read`; artifact `/Users/nadav/Desktop/Orchestrator/tmp/codex-browser-tools-live-proof/result.json`, created at `2026-05-27T05:18:48.190Z`. Elevated full `npm run compare:codex-side-panels -- --run-smoke --full` passed with 27 captures, `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; manifest `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-27T05:24:17.110Z`; comparison report `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T05:24:17.373Z`. Follow-up installed proof passed: `npm run pack:mac`, `npm run install:mac`, and elevated `npm run compare:codex-side-panels -- --run-smoke --full --installed --no-fail --smoke-out tmp/side-panel-visual-inventory-installed-full`; installed manifest `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-installed-full/manifest.json`, created at `2026-05-27T05:33:37.897Z`; comparison report `/Users/nadav/Desktop/Orchestrator/tmp/codex-side-panel-comparison/comparison-report.json`, created at `2026-05-27T05:33:38.151Z`, with `browserClientToolScreenshot=true` in the installed Browser capture.

Remaining: this closes the real-provider Browser dynamic screenshot gap. Native browser-use event streaming, image-content return semantics beyond text metadata/artifact paths, richer pointer/viewport actions, provider design-change application, and installed-app end-to-end Browser agent proof remain separate Browser backlog items.
