...
| Files / file viewer | Files is now a Workbench browser plus first-class preview/pinned file tabs. Host-aware ids, sticky folders, lazy directories, filtered ancestor projection, selected-row reveal, structured previews, notebook read-only controls, notebook output rendering, notebook `details`/`summary` cell disclosure shells, notebook execution-count/position header labels, notebook metadata title/description rendering, notebook output summary markdown, notebook raw-output disclosures, text/plain-before-JSON notebook MIME precedence, notebook Python-titled code snippets, PDF artifact header chrome, rich/source mode, Codex-shaped artifact import-kind/type metadata and source-option boundaries, `Cmd+F` focus routing to Files/source search when focused, source search, bounded content-hit search with line metadata, local annotations, selected-line reveal, selected-line actions, bounded gutter blame, shared action/context menu sections, shared Workbench file-tab action-menu sections, shared `PanelNotice` binary/unreadable fallback action states, and a shared Codex-like Workbench tree message primitive for Files empty/loading/no-match rows exist. | External PR/reviewer-backed comments/blame metadata is missing; global/provider-indexed workspace search is still basic; standalone/no-session split preview remains as fallback; full Codex artifact renderer fidelity for spreadsheet/document/slides/PDF remains incomplete, and notebook cells still need exact Codex spacing/output polish. | Focused Files smoke passes with `filesPdfPreview=true`, `filesArtifactTabModel=true`, `filesPreviewHeaderShared=true`, `filesArtifactPreviewControls=true`, `filesContentSearch=true`, `filesNotebookReadOnlyControls=true`, `filesNotebookOutputRendering=true`, `filesNotebookCellDisclosure=true`, `filesNotebookExecutionCount=true`, `filesNotebookCellMetadata=true`, `filesNotebookOutputSummaries=true`, `filesNotebookRawOutputDisclosure=true`, `filesNotebookCodeSnippet=true`, `filesActionMenuSharedSections=true`, `filesRowContextMenuSharedSections=true`, `workbenchFileTabActionMenuSharedSections=true`, `filesFallbackNoticeShared=true`, `filesNoResults=true`, and existing tree/search/source/preview gates. Latest notebook code-snippet evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779823811213.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779823811213.png`. Focused right-panel smoke also proves `Cmd+F` routes to workspace file search and Browser find from focused Workbench tabs. | Medium priority. Future work should focus on full artifact rendering, provider-backed comments/blame, provider/global indexed search, or exact Codex notebook styling rather than local tree polish. |
...
Remaining: exact notebook cell spacing and full provider-backed artifact metadata remain open; this slice closes the concrete raw-output/MIME behavior mismatch.
...
Remaining: this aligns the titled code-source shell; exact Codex cell spacing and full provider-backed artifact metadata remain open.
