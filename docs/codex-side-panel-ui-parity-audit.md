...
| Header and panel interaction | The runnable comparison now treats sidebar top inset, main session header ownership, Workbench right-panel header seam, and Terminal bottom-panel attachment as one shell contract. | Current Orchestrator and Codex are still at risk of drifting when a slice changes only one surface: sidebar top spacing, the titlebar/header row, right-panel chrome, and bottom-panel attachment can look locally correct but inconsistent together. Exact live Codex pixel spacing and panel animation timing remain unverified. | `npm run compare:codex-side-panels -- --run-smoke --full --no-fail` now includes a dedicated `Header and panel interaction` row backed by live screenshot evidence plus `sidebarTopInsetCodexLike=true`, `sessionHeaderInPrimaryColumn=true`, `rightPanelHeaderSeam=true`, and `terminalVisualHealthyContent=true` across the 23-surface visual inventory. Latest manifest: `/Users/nadav/Desktop/Orchestrator/tmp/side-panel-visual-inventory-current/manifest.json`, created at `2026-05-26T10:28:25.142Z`. | Treat this as a first-class parity workstream. Before claiming any sidebar, Workbench, main titlebar, or bottom Terminal change is complete, keep this row green and add a concrete mismatch if Codex shows a different header/panel relationship. |
