...
Implemented: Orchestrator artifact preview headers now group local open behavior behind a primary `Open file` toolbar button plus an adjacent `Open options` chevron/menu. JSON, CSV, DOCX, PDF, XLSX, PPTX, and IPYNB previews all expose `data-artifact-open-options="true"` and the menu contains `Open file` and `Reveal file`. The DOCX, PDF, spreadsheet, and slides custom headers keep their page/zoom controls while sharing the same Open/options affordance.

Verification: `node -c scripts/run-automated-ui-smoke.mjs`, `node -c scripts/run-codex-side-panel-comparison.mjs`, `pnpm exec tsc --noEmit`, `git diff --check`, and `npm run build` passed. Focused `npm run smoke:ui:auto -- --files` first hit the expected sandbox `listen EPERM 127.0.0.1`; the elevated rerun passed with `filesArtifactPreviewControls=true` and `filesArtifactOpenOptions=true`; evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779839394484.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779839394484.png`. The full visual inventory passed with 26 captures and no failures, including `filesArtifactOpenOptions=true` in the Files capture; latest Files evidence JSON: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779839546919.json`; screenshot: `/var/folders/bj/cxpn19xd78q4k1h9w4c_99700000gn/T/orchestrator-automated-ui-smoke-files-1779839546919.png`; manifest created at `2026-05-26T23:56:00.825Z`. `npm run compare:codex-side-panels -- --no-fail` passed with `fixture-covered=8`, `aligned=2`, `mismatch=0`, `blocked=0`, `needsSmoke=0`, and `needsProof=0`; report created at `2026-05-26T23:56:07.813Z`.
