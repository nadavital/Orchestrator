...
| PP-058 | Workbench and sidebar interiors still miss Codex-level styling parity. | A 2026-05-22 fine-grained audit of Codex bundle chunks found specific remaining gaps in typography, borders, radii, row weights, file tree metrics, search fields, browser tab chrome, settings rows, flyout sizing, hidden-webview lifecycle, and virtualized file/diff rendering. Current Review/Files/Browser screenshots pass smoke but still look heavier and more locally styled than Codex. | Use `docs/codex-side-panel-ui-parity-audit.md` as the styling source of truth. Implement the next slices around shared Workbench content primitives: 28px/13px virtualized tree rows, Codex-like search input, lighter badges/pills, fewer nested borders, first-class file tabs, browser lifecycle/preset parity, settings page primitives, and screenshot gates for each surface. | `Todo` |
