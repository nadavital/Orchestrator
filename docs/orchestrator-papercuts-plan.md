...
| PP-045 | Permission request cards need richer provider-normalized structure and pet parity. | Codex expands network/filesystem permission requests into explicit parts. Orchestrator preserves raw permissions generically, and pet overlay lacked `Allow Session`; other providers also emit tool/permission denials with provider-specific payloads. | First slice complete: added a shared permission request detail model for command/file/network/MCP/plan/tool approvals, rendered transcript permission cards as structured detail rows, and gave the pet overlay explicit `Allow Once`, `Allow Session`, and `Deny` actions backed by the existing allow-once/session resume paths. Remaining work: richer provider-specific payload fixtures, pending safety refresh on app focus, and live CUA verification of pet allow-session behavior against an actual paused run. | `In progress` |
