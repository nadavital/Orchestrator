...
| PP-030 | Workbench and Terminal need a shared app-shell tab controller. | Codex uses the same controller-shaped tab renderer for right and bottom panels; Orchestrator rendered Workbench tabs in `ContextSidebar` and Terminal tabs in `SessionPane` separately. | First renderer slices complete: Workbench and Terminal share `PanelTabStrip` chrome/behavior, drag reorder, and focus-routed close-active-tab support. Remaining work: promote this from shared renderer to full controller with tab registration, preview/pin state, generic tab state/reset, and cross-panel movement. | `In progress` |
