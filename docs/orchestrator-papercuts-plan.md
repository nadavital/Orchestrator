...
| PP-030 | Workbench and Terminal need a shared app-shell tab controller. | Codex uses the same controller-shaped tab renderer for right and bottom panels; Orchestrator rendered Workbench tabs in `ContextSidebar` and Terminal tabs in `SessionPane` separately. | First renderer slice complete: Workbench and Terminal share `PanelTabStrip` chrome and behavior. Remaining work: promote this from shared renderer to full controller with tab registration, close-active-tab support, preview/pin state, generic tab state/reset, cross-panel movement, and drag reorder hooks. | `In progress` |
