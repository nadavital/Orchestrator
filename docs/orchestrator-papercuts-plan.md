...
| PP-050 | The app window is nearly impossible to move/drag while a response is generating. | User testing reports native window dragging becomes effectively stuck during active response generation. This likely overlaps with PP-029 and PP-033: streaming updates, transcript rendering, and app-shell chrome may be putting too much pressure on the renderer/main process while the titlebar drag region needs to stay responsive. | Profile dragging during a simulated long stream, compare against Codex behavior, and isolate high-frequency streaming/render work from app chrome. Add a focused smoke/perf check that drags or measures titlebar responsiveness while a response is streaming, then fix the highest-pressure path before broader transcript refactors. | `Todo` |
