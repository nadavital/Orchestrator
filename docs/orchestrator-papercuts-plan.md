...
| PP-049 | Pinned chats should scroll with projects while remaining visually separate. | Pinned chats currently behave like a fixed area above the project list, so scrolling the Chat Sidebar can leave pinned chats detached from project content. | Keep pinned chats in their own section at the top of the Chat Sidebar, but place that section inside the same scroll container as projects so one scroll gesture moves pinned chats and project rows together. Verify with sidebar smoke and a screenshot/DOM check for the shared scroll container. | `Todo` |
...
- 2026-05-21: Started PP-046 settings data parity. Added archived chat inventory, restore, and permanent-delete APIs plus Data settings UI for refresh/restore/delete with confirmation. Verification passed: `npm run build`, escalated `npm run smoke:ui:auto -- --settings`, and `git diff --check`.
