...
| Notifications and app-server robustness | `app-server-manager-hooks-BpnIGmYe.js`, `app-server-manager-signals-BEaGjuc8.js`, `app-server-notification-debug-signals-B4ZABWlc.js`, `app-server-connection-state-COgfP2Bg.js` | main/renderer event bridge, pet overlay, system notifications, reconnect handling | Orchestrator has had crashes and EPIPE issues; Codex app-server state and notification diagnostics may show stronger patterns for reconnects, focus changes, and failure reporting. |
