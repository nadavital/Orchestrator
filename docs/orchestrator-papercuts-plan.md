...
| PP-058 | Workbench and sidebar interiors still miss Codex-level styling parity. | A 2026-05-22 fine-grained audit of Codex bundle chunks found specific remaining gaps in typography, borders, radii, row weights, file tree metrics, search fields, browser tab chrome, settings rows, flyout sizing, hidden-webview lifecycle, and virtualized file/diff rendering. Current Review/Files/Browser screenshots pass smoke but still look heavier and more locally styled than Codex. | Use `docs/codex-side-panel-ui-parity-audit.md` as the styling source of truth. First slice added shared Workbench search/list primitives across Review and Files, lightened badges and row selection, and made fixture-backed Workbench smokes reset their git workspace before each run. Remaining slices: first-class file tabs, Browser lifecycle/preset parity, settings page primitives, Chat Sidebar row density, flyout sizing, and screenshot gates for each surface. | `In progress` |
