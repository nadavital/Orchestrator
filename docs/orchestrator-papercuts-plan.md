...
| PP-054 | Git action buttons and review UI are still thin. | Review/files parsing improved, but Orchestrator did not have a Codex-level git action surface for stage/unstage, commit, branch, PR, revert/discard safeguards, or applying suggested changes. | First reversible slice complete: `FileChange` now carries staged/unstaged state, main-process git actions can stage/unstage selected paths, Review shows staged/unstaged badges plus compact Stage/Unstage controls, and the diff smoke clicks through stage/unstage. Remaining work: all-changes commit/branch/PR flow, discard/revert with confirmations, apply-suggestion actions, richer status summaries, and Codex comparison for review action placement. | `In progress` |
...
- 2026-05-21: Started PP-054 with reversible Git actions in Review. `gitManager.getChangedFiles` now preserves staged/unstaged status from porcelain output, main-process git action APIs stage/unstage path lists and return refreshed changed files, and Review exposes compact selected-file Stage/Unstage controls plus staged/unstaged row badges. Focused git tests cover staging and unstaging tracked plus untracked files. Focused Review smoke clicks Stage and Unstage selected, verifies the row/status state flips both ways, and screenshot review confirmed the toolbar remains compact.
