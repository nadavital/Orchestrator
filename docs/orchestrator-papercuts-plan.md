...
| PP-042 | Provider runtime transport must fail loudly and recoverably. | The old EPIPE crash was globally mitigated, but `CodexAppServerSession` still lacks child/stdin error listeners, write callbacks, pending-request rejection, and unexpected-exit failure events; similar semantics should apply to every process-backed provider runtime. | Harden provider transport writes/errors/exits through the runtime manager, make Codex app-server the first implementation, reject pending requests on close, and test mid-turn exit/EPIPE paths across provider lanes where possible. | `Todo` |
