...
| PP-051 | Composer typing still has bad latency while a response is streaming. | User testing after PP-050 reports typing while streaming is still bad. PP-008 isolated the first obvious composer/store churn, but that did not fully solve the active transcript/input contention. | Add a focused streaming-typing smoke before patching: measure key-to-text latency, textarea commits, `InputBar`/`SessionPane`/`ChatView` commits, and frame gaps while visible assistant text streams. Then isolate composer input from transcript streaming state and/or batch transcript rendering without delaying user input. | `Todo` |
...
| PP-054 | Git action buttons and review UI are still thin. | Review/files parsing improved, but Orchestrator does not yet have a Codex-level git action surface for stage/unstage, commit, branch, PR, revert/discard safeguards, or applying suggested changes. | Compare Codex git/review actions, add a provider-neutral Git actions service, expose safe status-aware buttons in Review/Workbench, and cover fixture repos plus smoke tests. | `Todo` |
