...
| P0 | Message editing and forks | Editing a previous user message was routed through local fork creation, and forks from pinned chats inherited local pinned state. | User-observed behavior from the Workbench UI; corrected in the agent-threads follow-up with focused smoke coverage. | Keep edit-as-draft separate from explicit fork creation. Editing should seed the current composer without switching chats; explicit forks from pinned source chats should start unpinned unless the user pins them. Guard with `chatUserMessageEditToDraft` and `chatMessageForkStartsUnpinnedFromPinnedSource`. |
