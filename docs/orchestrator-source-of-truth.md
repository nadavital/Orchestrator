...
| --- | --- | --- | --- | --- | --- |
| Sidebar row primitive | Dense single-line task row with title, right metadata, status/env slots, hover-revealed actions, keyboard/focus states, and hidden-on-hover metadata. | `SessionItem` is a taller title/preview row with always-present pin/menu slots and local status handling. | Row layout still feels app-specific and less deliberate. | `Partial` | Build a reusable `SessionRow` primitive with title, right meta, indicator, hover action, active state, keyboard focus, and stable layout slots. |
| Sidebar hover details | Rich delayed hover card shows title, project/folder, environment, worktree/cloud/remote context, and status details. | Native `title` tooltips only; no project/root/branch/provider details on hover. | Users cannot quickly answer "where is this chat?" without opening it. | `Planned` | Add hover card metadata: project name, root/cwd, branch/worktree, provider/model, status, last update, and useful warnings. |
| Sidebar environment identity | Compact local/cloud/worktree/remote icons with tooltips and full labels in hover cards. | Text `worktree` pill only when `session.useWorktree` is set. | Environment uses space-heavy text and misses local/remote/cloud distinctions. | `Partial` | Replace text pill with icon metadata; keep readable labels in hover card and accessibility labels. |
...
| Theme import/export | Portable `codex-theme-v1:` JSON theme string with validation. | No theme import/export. | Cannot share or round-trip themes. | `Planned` | Implement validated import/export using a compatible schema, with safe fallback if fields are missing. |
| Typography and motion | Raw UI/code font families, UI/code font sizes, font smoothing, pointer cursor, reduce motion. | Font presets and interface scale only. | Missing exact font sizing and motion/accessibility toggles. | `Partial` | Add raw font fields, numeric font sizes, font smoothing, pointer cursor, and reduce motion settings. |
| Token layer | Broad `text-token-*`, `bg-token-*`, `border-token-*`, and `--color-token-*` bridge across app surfaces. | Smaller app-specific CSS variables. | Harder to make reused components feel consistent. | `Partial` | Add Codex-like semantic token aliases on top of Orchestrator variables, then migrate high-use components. |
| Right panel architecture | Reusable app-shell panel with durable tab controller, closeable/reorderable/context-menu tabs, overflow polish, width/full-width persistence, empty states. | `ContextSidebar` derives fixed tabs from booleans; width is component-local; no durable tab model. | This is the main structural gap. | `Partial` | Replace boolean inspector state with per-session `rightPanel`: open, width, fullWidth, activeTabId, and durable tab list. |
| Right panel existing content | Existing panels can live as durable tabs before deeper redesign. | Plan, Diff, Agents, Extensions, Side Questions are custom tab buttons. | Existing content is usable but not in the right shell model. | `Partial` | Port current panels into tab kinds: `changes`, `plan`, `agents`, `extensions`, `side-chat`; preserve behavior first. |
| Review/diff surface | Review tab has changed-file tree/search, diff CSS, view modes, binary/no-content states, file actions, comments/action affordances. | `DiffPanel` has changed-file list and raw diff-ish preview. | Review feels much less like a code review surface. | `Partial` | Add changed-file tree/search, unified/split/wrap controls, binary/no-content states, open-in-editor, and better patch rendering. |
| Workspace/files browser | Side panel can browse workspace tree, search files, preview source/markdown/images/PDFs, copy paths, open with editor, add to chat. | No right-panel workspace browser; file cards and attachments exist elsewhere. | Users cannot inspect workspace from the side panel. | `Planned` | Add `Files` tab with file tree, search, preview, open/reveal/copy/add-to-chat actions, and persisted tree state. |
| Browser tab | First-class Browser tab with URL bar, empty state, local server affordances, screenshot, external open, cache/cookie/device/find controls. | No browser tab. | Missing a major Codex workbench surface. | `Planned` | Add browser tab after right shell and Files tab are stable; start with URL bar, empty state, navigation, screenshot/external open. |
...
| CP-003 | Sidebar organization | Persisted organize/sort modes, project actions, show more/less. | `Implemented` | CP-001 and CP-002. | Sidebar smoke covers organize menu options, Chronological list, By project restore, and show more/less is implemented for long project groups. Remaining project actions are deferred until the project menu/API pass. |
...
| CP-009 | Browser tab | URL bar, navigation, empty state, screenshot, external open, local server affordance. | `Implemented` | CP-005. | Inspector smoke opens the Browser tab, loads a local HTTP fixture inside the side-panel webview, verifies the page title, and captures a screenshot preview. Find/zoom/device/cache controls remain for the deeper Browser pass. |
| CP-010 | Side-chat tabs | `/btw` becomes durable side-chat tabs with independent composer/history and close cleanup. | `Implemented` | CP-005. | Inspector smoke opens multiple blank `/btw` side-chat tabs and verifies close cleanup. Message-level unread state remains for the deeper side-chat pass. |
| CP-011 | Terminal shell parity | Persisted bottom panel state, theme-aware terminal colors, optional multiple terminal tabs. | `Implemented` | Theme tokens; existing terminal. | Terminal smoke verifies multi-tab state persists across hide/show and records bottom-panel shell data hooks. Deeper terminal context menus/reorder remain future work. |
| CP-012 | Theme model v2 | Light/dark/system plus per-variant chrome/code theme settings and migration. | `Implemented` | Current settings schema. | Typecheck and settings smoke cover the expanded schema, light/dark/system resolver, Codex-style token bridge, and code/UI font-size tokens. Parser/editor tests land with CP-013/CP-014. |
| CP-013 | Custom theme editor | Editable accent/surface/foreground/contrast/semantic colors, font sizes, motion/cursor toggles. | `Implemented` | CP-012. | Settings smoke verifies the expanded settings surface still renders; controls now edit per-variant chrome colors, semantic colors, font sizes, and motion/cursor/font-smoothing toggles. |
...
| POL-006 | Codex-style settings taxonomy | Settings follow Codex's sectioned model while keeping Orchestrator behavior provider-neutral. | `Partial` | Execute CP-015 after theme model v2 is in place. |
| POL-007 | Theme parity | Theming supports per-variant custom colors, typography, semantic tokens, and import/export. | `Partial` | Execute CP-012 through CP-014. |
...
| POL-009 | Sidebar Codex parity | Chat rows expose project/branch/worktree/status metadata on hover and support richer organization modes. | `Partial` | Execute CP-001 through CP-003. |
