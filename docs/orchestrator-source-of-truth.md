...
| Transcript virtualization | Very long threads feel complete and scrollable without mounting every rendered message at once. | `Partial` | Older transcript chunks now lazy-hydrate when the user scrolls near the top, with anchor-preserving prepends and a manual "Show earlier" fallback. | Add true viewport virtualization with stable row measurement, overscan, and background prefetch around the active scroll window. |
...
- Lazy loading checkpoint: older transcript chunks hydrate automatically when the user scrolls near the top, preserving the viewport anchor so the transcript does not jump. This improves the long-thread experience without introducing a new dependency, but true row virtualization is still the next performance step.
