...
| Transcript virtualization | Very long threads feel complete and scrollable without mounting every rendered message at once. | `Planned` | Current transcript paging is the first step, but the user experience still exposes a manual "Show earlier" boundary. | Add viewport-based lazy rendering with stable row measurement, anchor-preserving prepend/append hydration, full-transcript search, and background prefetch around the active scroll window. |
