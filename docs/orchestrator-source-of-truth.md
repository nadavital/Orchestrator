...
| Antigravity SDK | SDK/runtime lane for Antigravity agent sessions, approvals, usage, and diagnostics. | `Research` | User-requested provider track added on 2026-05-19; no local SDK/API/package behavior has been verified in this repo yet. | Research the SDK surface, auth model, event stream, approval/user-input semantics, usage payloads, and local diagnostics before designing UI. |
...
| P8-005 | Antigravity SDK research spike. | `Research` | SDK/API/event/auth/usage surfaces are identified and mapped to Orchestrator provider runtime contracts. | Start with local/package/docs discovery; do not add runtime UI until semantics are verified. |
