...
- Verification passed for the transcript-virtualization checkpoint: `npx tsc -p tsconfig.node.json --noEmit`, `npx tsc -p tsconfig.web.json --noEmit`, `git diff --check`, `npm run smoke:ui:auto -- --session-switch` with bounded mounted virtual rows, `npm run smoke:ui:auto -- --transcript-layout`, and `npm run smoke:ui:auto -- --scroll`.
