...
export function providerManifest(providerId: string): ProviderManifest {
  ...
  const runtimes: ProviderRuntimeKind[] = ['headless']
  if (adapter.capabilities.interactiveCli) runtimes.push('interactive')
  ...
  return {
    ...
    name: def.name,
    runtimes,
    defaultRuntime: providerId === 'codex' ? 'app-server' : 'headless',
