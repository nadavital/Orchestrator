...
export function applyAppearance(
  ...
  document.documentElement.dataset.theme = resolvedVariant
  document.documentElement.dataset.appearanceTheme = appearanceModel?.appearanceTheme ?? legacyAppearanceTheme(appearance)
  ...
  document.documentElement.dataset.pointerCursors = appearanceModel?.usePointerCursors === false ? 'off' : 'on'
  document.documentElement.dataset.fontSmoothing = appearanceModel?.useFontSmoothing === false ? 'off' : 'on'
  document.documentElement.dataset.reducedMotion = appearanceModel?.reduceMotion ? 'on' : 'off'
  document.documentElement.dataset.codeTheme = resolvedVariant === 'mist'
    ? appearanceModel?.appearanceLightCodeThemeId ?? 'github-light'
    : appearanceModel?.appearanceDarkCodeThemeId ?? 'github-dark'
  document.documentElement.style.setProperty('--custom-accent', sanitizeHex(chromeTheme?.accent ?? customAccent, '#0a7cff'))
  document.documentElement.style.setProperty('--interface-scale', String(clamp(interfaceScale, 0.9, 1.12)))
  document.documentElement.style.setProperty('--font-ui-custom', fontStack(chromeTheme?.fonts?.ui ?? uiFont, 'ui'))
  document.documentElement.style.setProperty('--font-mono-custom', fontStack(chromeTheme?.fonts?.code ?? monoFont, 'mono'))
  document.documentElement.style.setProperty('--font-ui-size', `${clamp(appearanceModel?.sansFontSize ?? 13, 11, 18)}px`)
  document.documentElement.style.setProperty('--font-code-size', `${clamp(appearanceModel?.codeFontSize ?? 13, 11, 18)}px`)
